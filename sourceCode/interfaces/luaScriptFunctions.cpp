...
const SLuaVariables simLuaVariables[] = {
    ...
    // script states:
    {"sim.scriptstate_unloaded", sim_scriptstate_unloaded},
    {"sim.scriptstate_uninitialized", sim_scriptstate_uninitialized},
    {"sim.scriptstate_initialized", sim_scriptstate_initialized},
    {"sim.scriptstate_ended", sim_scriptstate_ended},
    {"sim.scriptstate_error", sim_scriptstate_error},
    {"sim.scriptstate_suspended", sim_scriptstate_suspended},
    // System callbacks
    ...
    {"sim.object_customsceneend", sim_object_customsceneend},
    ...
    {"sim.lang_python", sim_lang_python},
    ...
    {"sim.markertype_axes", sim_markertype_axes},
...
bool checkOneInputArgument(luaWrap_lua_State* L, int index, int type, std::string* errStr, int argOffset)
{
    ...
    if (type == lua_arg_handle)
    {
        if (!luaWrap_lua_ishandle(L, index))
        {
            if (errStr != nullptr)
            {
                std::string msg("bad argument #");
                msg += std::to_string(index - argOffset);
                msg += " (expecting a handle).";
                errStr->assign(msg.c_str());
            }
            return false; // error
        }
...
{ // make sure you have verified for correct args with checkInputArguments previously
    return int(fetchInt64Arg(L, index, defaultValue));
}
...
{ // make sure you have verified for correct args with checkInputArguments previously
    ...
    if ( (argCnt >= index) && (luaWrap_lua_isnonbuffertable(L, index)) )
    {
        ...
        outArr.resize(cnt);
        getInt32ArrayFromTable(L, index, cnt, outArr.data());
    }
}
...
{ // make sure you have verified for correct args with checkInputArguments previously
    ...
    if (arr.size() != 0)
        for (int x : arr) def.push_back(x);
    fetchHandleArrayArg(L, index, outArr, def);
}
...
{ // make sure you have verified for correct args with checkInputArguments previously
    ...
    if ( (argCnt >= index) && (luaWrap_lua_isnonbuffertable(L, index)) )
    {
        int cnt = int(luaWrap_lua_rawlen(L, index));
        outArr.resize(cnt);
        getHandleArrayFromTable(L, index, cnt, outArr.data());
    }
}
...
{ // make sure you have verified for correct args with checkInputArguments previously
    ...
    if ( (argCnt >= index) && (luaWrap_lua_isnonbuffertable(L, index)) )
    {
        int cnt = int(luaWrap_lua_rawlen(L, index));
        outArr.resize(cnt);
        getFloatArrayFromTable(L, index, cnt, outArr.data());
    }
}
...
{ // make sure you have verified for correct args with checkInputArguments previously
    ...
    if ( (argCnt >= index) && (luaWrap_lua_isnonbuffertable(L, index)) )
    {
        int cnt = int(luaWrap_lua_rawlen(L, index));
        outArr.resize(cnt);
        getDoubleArrayFromTable(L, index, cnt, outArr.data());
    }
}
...
{ // make sure you have verified for correct args with checkInputArguments previously
    ...
    {
        int cnt = int(luaWrap_lua_rawlen(L, index));
        getStringArrayFromTable(L, index, cnt, outArr);
    }
}
                            ...
                            std::string& raiseErrorWithMsg, CScriptCustomFunction* func /*=nullptr*/)
{
    ...
    {
        CSceneObject* obj = App::scene->sceneObjects->getObjectFromHandle(
...
{ // THIS FUNCTION SHOULD NOT LOCK THE API (AT LEAST NOT WHILE CALLING THE CALLBACK!!) SINCE IT IS NOT DIRECTLY
    ...
    { // new plugin functions
        ...
        CPlugin* plug = App::scenes->pluginContainer->getPluginFromName(namespaceAndVer.c_str());
        if (plug != nullptr)
        {
...
int _loadPlugin(luaWrap_lua_State* L)
{
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_string, 0))
    {
        ...
        plug = App::scenes->pluginContainer->loadAndInitPlugin(namespaceAndVersion.c_str(), it->getScriptUid());
        if (plug != nullptr)
        { // success
...
int _unloadPlugin(luaWrap_lua_State* L)
{
    ...
    if (luaWrap_lua_isnonbuffertable(L, 1))
    {
        ...
        if (pluginHandle >= 0)
        {
            CPlugin* plug = App::scenes->pluginContainer->getPluginFromHandle(pluginHandle);
            if ((plug != nullptr) && (plug->hasDependency(it->getScriptUid())))
            {
                errorString.clear();
                ...
                App::scenes->pluginContainer->deinitAndUnloadPlugin(pluginHandle, it->getScriptUid(), (options & 1) != 0);
            }
        }
    }
...
int _registerCodeEditorInfos(luaWrap_lua_State* L)
{
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_string, 0, lua_arg_string, 0))
        App::scenes->codeEditorInfos->setInfo(luaWrap_lua_tostring(L, 1), luaWrap_lua_tostring(L, 2),
                                                      &errorString);
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_string, 0))
    {
        ...
        if (cmd.compare("getfiles") == 0)
        {
                                    ...
                                    lua_arg_string, 0))
            {
                ...
                pushStringArrayAsTable(L, nstrs);
                pushStringArrayAsTable(L, pstrs);
                LUA_END(2);
            }
        }
...
int _simHandleVisionSensor(luaWrap_lua_State* L)
{
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_integer, 0))
    {
        ...
        if (retVal != -1)
        {
            ...
            {
                pushInt32ArrayAsTable(L, 0, nullptr); // return at least 2 aux packets, even empty
                tableCount++;
            }
...
int _simSetVisionSensorImg(luaWrap_lua_State* L)
{
    ...
    {
        int sensorHandle = luaToInt(L, 1);
        CSceneObject* it = App::scene->sceneObjects->getObjectFromHandle(sensorHandle);
        if (it != nullptr)
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_number, 0))
    {
        ...
        if ((res == 0) || (res == 2))
        {
            ...
            if ((res == 0) || (res == 2))
            {
                ...
                {
                    if (res == 2)
                        getInt32ArrayFromTable(L, 4, 2, size);
                    int resolution[2];
...
int _simCheckProximitySensor(luaWrap_lua_State* L)
{
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_number, 0, lua_arg_number, 0))
    {
        ...
        CProxSensor* it = App::scene->sceneObjects->getProximitySensorFromHandle(handle);
        if (it != nullptr)
        {
...
int _simSetJointTargetPosition(luaWrap_lua_State* L)
{
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_number, 0, lua_arg_number, 0))
    {
        ...
        if (res >= 0)
        {
            ...
            if (res == 2)
            {
                CJoint* joint = App::scene->sceneObjects->getJointFromHandle(h);
                if (joint != nullptr)
                {
                ...
                if (joint != nullptr)
                {
                        ...
                        (joint->getJointMode() == sim_jointmode_dynamic))
                    {
                        if ((joint->getJointMode() == sim_jointmode_kinematic) || joint->getDynSmoothMotionProfile())
                        {
                            double maxVelAccelJerk[3];
                            joint->getMaxVelAccelJerk(maxVelAccelJerk);
                            getDoubleArrayFromTable(L, 3, std::min<size_t>(luaWrap_lua_rawlen(L, 3), 2),
                                                maxVelAccelJerk + 1);
                            joint->setMaxVelAccelJerk(maxVelAccelJerk);
                        }
                    }
...
int _simCheckCollision(luaWrap_lua_State* L)
{
    ...
    LUA_RAISE_ERROR_OR_YIELD_IF_NEEDED(); // we might never return from this!
    luaWrap_lua_pushinteger(L, retVal);
    pushInt32ArrayAsTable(L, 2, collidingIds);
    LUA_END(2);
}
...
int _simCheckDistance(luaWrap_lua_State* L)
{
    ...
    LUA_RAISE_ERROR_OR_YIELD_IF_NEEDED(); // we might never return from this!
    luaWrap_lua_pushinteger(L, retVal);
    pushDoubleArrayAsTable(L, 7, distanceData);
    pushInt32ArrayAsTable(L, 2, tb);
    LUA_END(3);
}
...
int _simGetSimulatorMessage(luaWrap_lua_State* L)
{
    ...
    pushInt32ArrayAsTable(L, 4, auxVals);
    pushDoubleArrayAsTable(L, aux2Cnt, aux2Vals);
    LUA_END(3);
}
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_integer, 0, lua_arg_string, 0, lua_arg_string, 0))
    {
        ...
        if (streamName.size() != 0)
        {
            ...
            if ((res == 0) || (res == 2))
            {
                ...
                if (res == 2)
                    getFloatArrayFromTable(L, 5, 3, col);
                if ((res == 0) || (res == 2))
                {
                ...
                if (curveName.size() != 0)
                {
                    ...
                    if ((res == 0) || (res == 2))
                    {
                        ...
                        if ((res == 0) || (res == 2))
                        {
                            ...
                            if (res == 2)
                                getFloatArrayFromTable(L, 8, 3, col);
                            if ((res == 0) || (res == 2))
                            {
                                int curveWidth = 2;
...
int _simSetObjectPose(luaWrap_lua_State* L)
{
    ...
    if (luaWrap_lua_isnonbuffertable(L, 2))
    {
        ...
        {
            double coord[7];
            getDoubleArrayFromTable(L, 2, 7, coord);
            int rel = sim_handle_world;
            if (luaWrap_lua_isinteger(L, 3))
                rel = luaToInt(L, 3);
    ...
    { // old
        ...
        {
            double arr[7];
            getDoubleArrayFromTable(L, 3, 7, arr);
...
int _simSetObjectChildPose(luaWrap_lua_State* L)
{
    ...
    {
        double arr[7];
        getDoubleArrayFromTable(L, 2, 7, arr);
...
int _simGetIntArray2Property(luaWrap_lua_State* L)
{
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_integer, 0, lua_arg_string, 0))
    {
        ...
        {
            pushInt32ArrayAsTable(L, 2, pValue);
            LUA_END(1);
        }
        if (noError)
        {
...
int _simGetVector3Property(luaWrap_lua_State* L)
{
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_integer, 0, lua_arg_string, 0))
    {
        ...
        {
            pushDoubleArrayAsTable(L, 3, pValue);
            LUA_END(1);
        }
        if (noError)
        {
...
int _simGetQuaternionProperty(luaWrap_lua_State* L)
{
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_integer, 0, lua_arg_string, 0))
    {
        ...
        {
            pushDoubleArrayAsTable(L, 4, pValue);
            LUA_END(1);
        }
        if (noError)
        {
...
int _simGetPoseProperty(luaWrap_lua_State* L)
{
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_integer, 0, lua_arg_string, 0))
    {
        ...
        {
            pushDoubleArrayAsTable(L, 7, pValue);
            LUA_END(1);
        }
        if (noError)
        {
...
int _simSetFloatArrayProperty(luaWrap_lua_State* L)
{
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_integer, 0, lua_arg_string, 0, lua_arg_number, -1))
    {
        ...
        std::vector<double> v;
        v.resize(cnt);
        getDoubleArrayFromTable(L, 3, cnt, v.data());
        bool noError = false;
        if (luaWrap_lua_isnonbuffertable(L, 4))
        {
            CInterfaceStack* stack = App::scenes->interfaceStackContainer->createStack();
...
int _simGetFloatArrayProperty(luaWrap_lua_State* L)
{
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_integer, 0, lua_arg_string, 0))
    {
        ...
        if (res > 0)
        {
            pushDoubleArrayAsTable(L, pValueL, pValue);
            delete[] pValue;
            LUA_END(1);
        }
        if (noError)
...
int _simSetIntArrayProperty(luaWrap_lua_State* L)
{
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_integer, 0, lua_arg_string, 0, lua_arg_integer, -1))
    {
        ...
        std::vector<int> v;
        v.resize(cnt);
        getInt32ArrayFromTable(L, 3, cnt, v.data());
        bool noError = false;
        if (luaWrap_lua_isnonbuffertable(L, 4))
        {
            CInterfaceStack* stack = App::scenes->interfaceStackContainer->createStack();
...
int _simGetIntArrayProperty(luaWrap_lua_State* L)
{
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_integer, 0, lua_arg_string, 0))
    {
        ...
        if (res > 0)
        {
            pushInt32ArrayAsTable(L, pValueL, pValue);
            delete[] pValue;
            LUA_END(1);
        }
        if (noError)
...
{ // ALL handle items have to be of the same type
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_integer, 0, lua_arg_string, 0, lua_arg_integer, -1))
    {
        ...
        v.resize(cnt);
        getInt64ArrayFromTable(L, 3, cnt, v.data());
        bool noError = false;
        if (luaWrap_lua_isnonbuffertable(L, 4))
        {
            CInterfaceStack* stack = App::scenes->interfaceStackContainer->createStack();
...
{ // ALL handle items have to be of the same type
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_integer, 0, lua_arg_string, 0))
    {
        ...
        if (res > 0)
        {
            pushInt64ArrayAsTable(L, pValueL, pValue);
            delete[] pValue;
            LUA_END(1);
        }
        if (noError)
...
int _simRemoveObjects(luaWrap_lua_State* L)
{
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_number, -1))
    {
        ...
        {
            handles.resize(cnt);
            getInt32ArrayFromTable(L, 1, cnt, &handles[0]);
            bool delayed = false;
...
int _simGetJointInterval(luaWrap_lua_State* L)
{
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_number, 0))
    {
        ...
        if (CALL_C_API(simGetJointInterval, luaToInt(L, 1), &cyclic, interval) == 1)
        {
            ...
            pushDoubleArrayAsTable(L, 2, interval);
            LUA_END(2);
        }
    }
...
int _simSetJointInterval(luaWrap_lua_State* L)
{
    ...
    {
        double interval[2];
        getDoubleArrayFromTable(L, 3, 2, interval);
        ...
        { // loading from file:
            ...
            if (CFileOperations::loadModel(path.c_str(), false, false, nullptr, onlyThumbnails, forceAsCopy, &infoStr, &errorString))
            {
                ...
                if (onlyThumbnails)
                {
                    ...
                    bool opRes = App::scene->environment->modelThumbnail_notSerializedHere
                    ...
                    retVal = App::scene->sceneObjects->getLastSelectionHandle();
            }
            setLastInfo(infoStr.c_str());
        }
        else
...
int _simSaveModel(luaWrap_lua_State* L)
{
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_integer, 0, lua_arg_string | lua_arg_optional, 0))
    {
        ...
        { // here we don't save to file, but to buffer:
            if (!App::scene->environment->getSceneLocked())
            {
                CSceneObject* it = App::scene->sceneObjects->getObjectFromHandle(model);
                if (it != nullptr)
                {
                    if (it->getModelBase())
                    {
...
int _simGetObjectSel(luaWrap_lua_State* L)
{
    ...
    if (sel != nullptr)
    {
        pushInt32ArrayAsTable(L, selSize, sel);
        delete[] sel;
        LUA_END(1);
    }
    ...
    if (argCnt >= 1)
    {
        if (luaWrap_lua_isnonbuffertable(L, 1))
        {
            ...
            if ((objCnt == 0) || checkInputArguments(L, &errorString, argOffset, lua_arg_integer, objCnt))
            {
                ...
                getInt32ArrayFromTable(L, 1, objCnt, &objectHandles[0]);
...
int _simTest(luaWrap_lua_State* L)
{
    ...
    if (checkInputArguments(L, nullptr, argOffset, lua_arg_string, 0))
    {
        ...
        {
            std::string r = App::scenes->pluginContainer->dyn_generateMjcfFile();
        ...
        { // pt1,pt2,vertices,indices(can be {}),maxEdge,debugShape
            C3Vector pt1, pt2;
            getDoubleArrayFromTable(L, 2, 3, pt1.data);
            getDoubleArrayFromTable(L, 3, 3, pt2.data);
            int vl = 3 * ((int)luaWrap_lua_rawlen(L, 4) / 3);
            std::vector<double> vertices;
            vertices.resize(vl);
            getDoubleArrayFromTable(L, 4, vl, vertices.data());
            int il = 3 * ((int)luaWrap_lua_rawlen(L, 5) / 3);
            std::vector<int> indices;
            indices.resize(il);
            getInt32ArrayFromTable(L, 5, il, indices.data());
            ...
            pushDoubleArrayAsTable(L, path.size(), path.data());
        ...
        if (cmd.compare("getModelState") == 0)
        {
            ...
            std::string state = App::scene->sceneObjects->getModelState(h, p);
...
#ifdef SIM_WITH_GUI
    if (App::scenes->pluginContainer->isCodeEditorPluginAvailable() && (GuiApp::mainWindow != nullptr))
    {
        if (checkInputArguments(L, &errorString, argOffset, lua_arg_string, 0, lua_arg_string, 0))
        {
    ...
    if (App::scenes->pluginContainer->isCodeEditorPluginAvailable() && (GuiApp::mainWindow != nullptr))
    {
        if (checkInputArguments(L, &errorString, argOffset, lua_arg_number, 0))
        {
            ...
            if (GuiApp::mainWindow->codeEditorContainer->close(handle, posAndSize, &txt, nullptr))
            {
                ...
                pushInt32ArrayAsTable(L, 2, posAndSize + 0);
                pushInt32ArrayAsTable(L, 2, posAndSize + 2);
                LUA_END(3);
            }
        }
    }
...
int _simCopyPasteObjects(luaWrap_lua_State* L)
{
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_integer, 1, lua_arg_integer | lua_arg_optional,0))
    {
        ...
        {
            pushInt32ArrayAsTable(L, objectHandles.size(), &objectHandles[0]);
            LUA_END(1);
        }
    }
...
int _simSaveImage(luaWrap_lua_State* L)
{
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_string, 0, lua_arg_number, 2, lua_arg_string, 0, lua_arg_number | lua_arg_optional, 0, lua_arg_number | lua_arg_optional, 0))
    {
        ...
        int res[2];
        getInt32ArrayFromTable(L, 2, 2, res);
...
int _simLoadImage(luaWrap_lua_State* L)
{
    ...
    { // arg order switched on 14.11.2025
        ...
        if (img != nullptr)
        {
            ...
            delete[]((char*)img);
            pushInt32ArrayAsTable(L, 2, resol);
            LUA_END(2);
        }
    }
...
int _simGetScaledImage(luaWrap_lua_State* L)
{
                            ...
                            0))
    {
        ...
        int resIn[2];
        getInt32ArrayFromTable(L, 2, 2, resIn);
        int resOut[2];
        getInt32ArrayFromTable(L, 3, 2, resOut);
        ...
        if (int(dataLength) >= resIn[0] * resIn[1] * channelsIn)
        {
            if ((resIn[0] > 0) && (resIn[1] > 0) && (resOut[0] > 0) && (resOut[1] > 0))
            {
                ...
                if (imgOut != nullptr)
                {
                    ...
                    delete[]((char*)imgOut);
                    pushInt32ArrayAsTable(L, 2, resOut);
                    LUA_END(2);
                }
            }
            else
...
int _simTransformImage(luaWrap_lua_State* L)
{
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_string, 0, lua_arg_number, 2, lua_arg_number, 0))
    {
        ...
        char* data = ((char*)luaWrap_lua_tobuffer(L, 1, &dataLength));
        int resol[2];
        getInt32ArrayFromTable(L, 2, 2, resol);
...
int _simPackInt64Table(luaWrap_lua_State* L)
{
    TRACE_LUA_API;
    LUA_START("sim.packInt64Table");
        ...
        if (luaWrap_lua_isnonbuffertable(L, 1))
        {
            ...
            if ((res == 0) || (res == 2))
            {
                ...
                if ((res == 0) || (res == 2))
                {
                    ...
                    if (count > 0)
                    {
                        char* data = new char[sizeof(int64_t) * count];
                        for (int i = 0; i < count; i++)
                        {
                            luaWrap_lua_rawgeti(L, 1, i + 1 + startIndex);
                            int64_t v = luaWrap_lua_tointeger(L, -1);
                            data[8 * i + 0] = ((char*)&v)[0];
                            data[8 * i + 1] = ((char*)&v)[1];
                            data[8 * i + 2] = ((char*)&v)[2];
                            data[8 * i + 3] = ((char*)&v)[3];
                        ...
                        luaWrap_lua_pushbuffer(L, (const char*)data, count * sizeof(int64_t));
                        delete[] data;
                        LUA_END(1);
                    }
                }
                ...
                if ((res == 0) || (res == 2))
                {
                    ...
                    else
                    {
                        if ((startIndex >= 0) && (startIndex < packetCount))
                        {
                            ...
                            pushInt32ArrayAsTable(L, count, theInts);
                            delete[] theInts;
                            LUA_END(1);
                        }
                    }
...
int _simUnpackInt64Table(luaWrap_lua_State* L)
{
    TRACE_LUA_API;
    LUA_START("sim.unpackInt64Table");
        ...
        if ((res == 0) || (res == 2))
        {
            ...
            if ((res == 0) || (res == 2))
            {
                ...
                if ((res == 0) || (res == 2))
                {
                    ...
                    size_t dataLength;
                    char* data = ((char*)luaWrap_lua_tobuffer(L, 1, &dataLength)) + additionalCharOffset;
                    dataLength = sizeof(int64_t) * ((dataLength - additionalCharOffset) / sizeof(int64_t));
                    ...
                    else
                    {
                        if ((startIndex >= 0) && (startIndex < packetCount))
                        {
                            ...
                            int64_t* theInts = new int64_t[count];
                            for (int i = 0; i < int(count); i++)
                            {
                                int64_t v;
                            ...
                            pushInt64ArrayAsTable(L, count, theInts);
                            delete[] theInts;
                            LUA_END(1);
                        }
                    }
                ...
                if ((res == 0) || (res == 2))
                {
                    ...
                    else
                    {
                        if ((startIndex >= 0) && (startIndex < packetCount))
                        {
                            ...
                            pushUInt32ArrayAsTable(L, count, theInts);
                            delete[] theInts;
                            LUA_END(1);
                        }
                    }
                ...
                if ((res == 0) || (res == 2))
                {
                    ...
                    else
                    {
                        if ((startIndex >= 0) && (startIndex < packetCount))
                        {
                            ...
                            pushFloatArrayAsTable(L, count, theFloats);
                            delete[] theFloats;
                            LUA_END(1);
                        }
                    }
                ...
                if ((res == 0) || (res == 2))
                {
                    ...
                    else
                    {
                        if ((startIndex >= 0) && (startIndex < packetCount))
                        {
                            ...
                            pushDoubleArrayAsTable(L, count, theDoubles);
                            delete[] theDoubles;
                            LUA_END(1);
                        }
                    }
            ...
            if ((res == 0) || (res == 2))
            {
                ...
                else
                {
                    if ((startIndex >= 0) && (startIndex < packetCount))
                    {
                        ...
                        pushInt32ArrayAsTable(L, count, theBytes);
                        delete[] theBytes;
                        LUA_END(1);
                    }
                }
                ...
                if ((res == 0) || (res == 2))
                {
                    ...
                    else
                    {
                        if ((startIndex >= 0) && (startIndex < packetCount))
                        {
                            ...
                            pushInt32ArrayAsTable(L, count, theWords);
                            delete[] theWords;
                            LUA_END(1);
                        }
                    }
            ...
            if (dataLength != 0)
            {
                if (outFormat == sim_buffer_split)
                {
                    ...
                    if (splitSize > 0)
                    {
                        ...
                        pushBufferArrayAsTable(L, stringTable);
                        LUA_END(1);
                    }
                }
...
int _simCombineRgbImages(luaWrap_lua_State* L)
{
                            ...
                            lua_arg_number, 0))
    {
        ...
        int res1[2];
        int res2[2];
        getInt32ArrayFromTable(L, 2, 2, res1);
        getInt32ArrayFromTable(L, 4, 2, res2);
        int op = luaToInt(L, 5);
            ...
            else
            {
                if ((checkOneGeneralInputArgument(L, 5, lua_arg_number, 0, false, false, &errorString, argOffset) == 2) && (checkOneGeneralInputArgument(L, 6, lua_arg_number, 0, false, false, &errorString, argOffset) == 2))
                {
                    ...
                    if (!luaWrap_lua_isnil(L, 4))
                    {
                        ...
                        getInt32ArrayFromTable(L, 4, paramLen, intParams);
                        getDoubleArrayFromTable(L, 4, paramLen, floatParams);
                    ...
                    if (okToGo)
                    {
                        ...
                        if (res > 0)
                        {
                            if (res == 2)
                            {
                                getFloatArrayFromTable(L, 7, 3, ambientC);
                                ambient = ambientC;
                            }
                                ...
                                if (res > 0)
                                {
                                    ...
                                    if (okToGo)
                                    {
                                        if (res > 0)
                                        {
                                            if (res == 2)
                                            {
                                                getFloatArrayFromTable(L, 9, 3, specularC);
                                                specular = specularC;
                                            }
                                            ...
                                            if (okToGo)
                                            {
                                                if (res > 0)
                                                {
                                                    if (res == 2)
                                                    {
                                                        getFloatArrayFromTable(L, 10, 3, emissionC);
                                                        emission = emissionC;
                                                    }
                                                }
                                            }
                        ...
                        if (okToGo)
                        {
                            if (!App::scene->simulation->isSimulationRunning())
                                ...
                                retVal = App::scenes->pluginContainer->dyn_addParticleObject(
...
int _simRemoveParticleObject(luaWrap_lua_State* L)
{
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_number, 0))
    {
        if (App::scenes->pluginContainer->dyn_removeParticleObject(luaToInt(L, 1)) != 0)
            retVal = 1;
    }
...
int _simAddParticleObjectItem(luaWrap_lua_State* L)
{
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_number, 0))
    {
        ...
        int d = 6 + App::scenes->pluginContainer->dyn_getParticleObjectOtherFloatsPerItem(particleObjHandle);
        ...
        if (res == 2)
        {
            ...
            getDoubleArrayFromTable(L, 2, d, vertex);

            if (App::scenes->pluginContainer->dyn_addParticleObjectItem(
                    particleObjHandle, vertex, App::scene->simulation->getSimulationTime()) != 0)
                retVal = 1;
            else
                errorString = SIM_ERROR_OBJECT_INEXISTANT;
        }
        else
        {
            ...
            {
                if (App::scenes->pluginContainer->dyn_addParticleObjectItem(
                        particleObjHandle, nullptr, App::scene->simulation->getSimulationTime()) != 0)
                    retVal = 1;
                else
                    errorString = SIM_ERROR_OBJECT_INEXISTANT;
            }
...
int _simGetVelocity(luaWrap_lua_State* L)
{
    ...
    LUA_RAISE_ERROR_OR_YIELD_IF_NEEDED(); // we might never return from this!
    pushDoubleArrayAsTable(L, 3, linVel);
    pushDoubleArrayAsTable(L, 3, angVel);
    LUA_END(2);
}
...
int _simAddForce(luaWrap_lua_State* L)
{
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_number, 0, lua_arg_number, 3, lua_arg_number, 3))
    {
        double r[3];
        double f[3];
        getDoubleArrayFromTable(L, 2, 3, r);
        getDoubleArrayFromTable(L, 3, 3, f);
        ...
        if (!err)
        {
            ...
            if (!err)
            {
                ...
                if (res == 2)
                {
                    ...
                    getFloatArrayFromTable(L, 4, floatsInTableExpected, &rgbData[0]);
    ...
    { // First arg ok
        ...
        if (!err)
        {
            ...
            if (!err)
            {
                ...
                {
                    luaWrap_lua_pushinteger(L, retVal);
                    pushFloatArrayAsTable(L, res, &rgbData[0]);
                    LUA_END(2);
                }
            }
        }
...
int _simSerialOpen(luaWrap_lua_State* L)
{
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_string, 0, lua_arg_number, 0))
    {
        ...
        retVal = App::scenes->serialPortContainer->serialPortOpen(true, portName, baudrate);
    }
#endif
...
{ // problematic for remote API and in general: function returns different return value count
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_number, 0, lua_arg_number, 0, lua_arg_number, 0))
    {
        ...
        if (res == 1)
        {
            pushInt32ArrayAsTable(L, 2, collidingObjects);
            pushDoubleArrayAsTable(L, 3, contactInfo);
            pushDoubleArrayAsTable(L, 3, contactInfo + 3);
            pushDoubleArrayAsTable(L, 3, contactInfo + 6);
        }
        else
        {
            pushInt32ArrayAsTable(L, 0, nullptr);
            pushDoubleArrayAsTable(L, 0, nullptr);
            pushDoubleArrayAsTable(L, 0, nullptr);
            pushDoubleArrayAsTable(L, 0, nullptr);
        }
        LUA_END(4);
                ...
                if (!error)
                {
                    ...
                    if (luaWrap_lua_isnonbuffertable(L, -1))
                    {
                        ...
                        if (checkOneGeneralInputArgument(L, luaWrap_lua_gettop(L), lua_arg_number, vl, false, false, &errorString, argOffset) == 2)
                        {
                            ...
                            getDoubleArrayFromTable(L, luaWrap_lua_gettop(L), vl, vertices[i]);
                        }
                        else
                            error = true;
                    }
                ...
                if (!error)
                {
                    ...
                    if (luaWrap_lua_isnonbuffertable(L, -1))
                    {
                        ...
                        if (checkOneGeneralInputArgument(L, luaWrap_lua_gettop(L), lua_arg_number, vl, false, false, &errorString, argOffset) == 2)
                        {
                            ...
                            getInt32ArrayFromTable(L, luaWrap_lua_gettop(L), vl, indices[i]);
                        }
                        else
                            error = true;
                    }
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_integer, 0, lua_arg_number, 0))
    {
        ...
        if (res == 2)
        {
            ...
            if (res == 2)
            {
                ...
                std::vector<int> indices;
                indices.resize(il);
                getDoubleArrayFromTable(L, 3, vl, vertices.data());
                getInt32ArrayFromTable(L, 4, il, indices.data());
                ...
                if (res == 2)
                    getInt32ArrayFromTable(L, 8, 2, resol);
...
int _simGetShapeMesh(luaWrap_lua_State* L)
{
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_number, 0))
    {
        ...
        if (result > 0)
        {
            pushDoubleArrayAsTable(L, verticesSize, vertices);
            pushInt32ArrayAsTable(L, indicesSize, indices);
            pushDoubleArrayAsTable(L, indicesSize * 3, normals);
            delete[] vertices;
            delete[] indices;
            delete[] normals;
            LUA_END(3);
...
int _simGetShapeBB(luaWrap_lua_State* L)
{
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_integer, 0))
    {
        CSceneObject* it = App::scene->sceneObjects->getObjectFromHandle(luaToInt(L, 1));
        if (it != nullptr)
        {
            if (it->getObjectType() == sim_sceneobject_shape)
            {
                ...
                double p[7];
                ltr.getData(p, true);
                pushDoubleArrayAsTable(L, 3, hs.data);
                pushDoubleArrayAsTable(L, 7, p);
                LUA_END(2);
            }
            else
...
int _simCreatePrimitiveShape(luaWrap_lua_State* L)
{
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_integer, 0, lua_arg_number, 3))
    {
        ...
        double sizes[3];
        getDoubleArrayFromTable(L, 2, 3, sizes);
...
int _simCreateHeightfieldShape(luaWrap_lua_State* L)
{
                            ...
                            lua_arg_number, 0))
    {
        ...
        if (res == 2)
        {
            ...
            getDoubleArrayFromTable(L, 6, xPointCount * yPointCount, heights);
            retVal =
...
int _simCreateDetachedScript(luaWrap_lua_State* L)
{
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_integer, 0, lua_arg_string, 0, lua_arg_string | lua_arg_optional, 0, lua_arg_integer | lua_arg_optional, 0))
    {
        ...
        if (scriptType == sim_scripttype_addon)
            retVal = App::scenes->addOnScriptContainer->createAddOn(lang.c_str(), code.c_str());
        else
...
int _simRemoveDetachedScript(luaWrap_lua_State* L)
{
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_integer, 0))
    {
        int64_t scriptHandle = fetchInt64Arg(L, 1);
        if (!App::scenes->addOnScriptContainer->removeAddOn(scriptHandle))
...
int _simRuckigPos(luaWrap_lua_State* L)
{
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_number, 0, lua_arg_number, 0, lua_arg_number, 0))
    {
                                ...
                                lua_arg_number, dofs * 2))
        {
            ...
            getDoubleArrayFromTable(L, 4, dofs * 3, &currentPosVelAccel[0]);

            std::vector<double> maxVelAccelJerk;
            maxVelAccelJerk.resize(maxVelAccelJerkCnt);
            getDoubleArrayFromTable(L, 5, maxVelAccelJerkCnt, &maxVelAccelJerk[0]);
            ...
            getCharArrayFromTable(L, 6, dofs, &selection[0]);
...
int _simRuckigVel(luaWrap_lua_State* L)
{
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_number, 0, lua_arg_number, 0, lua_arg_number, 0))
    {
                                ...
                                lua_arg_number, dofs))
        {
            ...
            getDoubleArrayFromTable(L, 5, maxAccelJerkCnt, &maxAccelJerk[0]);
...
int _simRuckigStep(luaWrap_lua_State* L)
{
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_number, 0, lua_arg_number, 0))
    {
        int handle = luaToInt(L, 1);
        double timeStep = luaToDouble(L, 2);
        int dofs = App::scenes->pluginContainer->ruckigPlugin_dofs(handle);
        if (dofs < 0)
    ...
    if (newPosVelAccel.size() > 0)
        pushDoubleArrayAsTable(L, newPosVelAccel.size(), &newPosVelAccel[0]);
    else
...
int _simGetObjectQuaternion(luaWrap_lua_State* L)
{
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_number, 0, lua_arg_integer | lua_arg_optional, 0))
    {
        ...
        {
            pushDoubleArrayAsTable(L, 4, coord);
            LUA_END(1);
        }
    }
...
int _simSetObjectQuaternion(luaWrap_lua_State* L)
{
    ...
    if (luaWrap_lua_isnonbuffertable(L, 2))
    {
        ...
        {
            double coord[4];
            getDoubleArrayFromTable(L, 2, 4, coord);
            int rel = sim_handle_world;
            if (luaWrap_lua_isinteger(L, 3))
                rel = luaToInt(L, 3);
    ...
    { // old
        ...
        {
            double coord[4];
            getDoubleArrayFromTable(L, 3, 4, coord);
...
int _simGetQuaternionFromMatrix(luaWrap_lua_State* L)
{
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_number, 12))
    {
        ...
        {
            pushDoubleArrayAsTable(L, 4, quaternion);
            LUA_END(1);
        }
    }
...
int _simGetShapeInertia(luaWrap_lua_State* L)
{
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_number, 0))
    {
        ...
        if (result != -1)
        {
            ...
            pushDoubleArrayAsTable(L, 12, transformation);
            LUA_END(2);
        }
    }
...
int _simSetShapeInertia(luaWrap_lua_State* L)
{
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_number, 0, lua_arg_number, 9, lua_arg_number, 12))
    {
        int handle = luaToInt(L, 1);
        double inertiaMatrix[9];
        getDoubleArrayFromTable(L, 2, 9, inertiaMatrix);
        double transformation[12];
        getDoubleArrayFromTable(L, 3, 12, transformation);
...
int _simGenerateShapeFromPath(luaWrap_lua_State* L)
{
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_number, 14, lua_arg_number, 4))
    {
        ...
        getDoubleArrayFromTable(L, 1, luaWrap_lua_rawlen(L, 1), &ppath[0]);
        getDoubleArrayFromTable(L, 2, luaWrap_lua_rawlen(L, 2), &section[0]);
        int options = 0;
        ...
        if (res >= 0)
        {
            ...
            if (res >= 0)
            {
                ...
                if (res == 2)
                {
                    getDoubleArrayFromTable(L, 4, 3, tmp);
                    zvect = tmp;
                }
...
int _simGetClosestPosOnPath(luaWrap_lua_State* L)
{
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_number, 3, lua_arg_number, 1, lua_arg_number, 3))
    {
        ...
        if (pathLS <= pathS / 3)
        {
            ...
            getDoubleArrayFromTable(L, 1, pathS, &ppath[0]);
            getDoubleArrayFromTable(L, 2, pathLS, &pathLengths[0]);
            getDoubleArrayFromTable(L, 3, 3, absPt);
...
int _simPushUserEvent(luaWrap_lua_State* L)
{
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_string, 0, lua_arg_integer, 0, lua_arg_integer, 0))
    {
        ...
        int64_t uid = luaWrap_lua_tointeger(L, 3);
        if (luaWrap_lua_isnonbuffertable(L, 4))
        {
...
int _simGroupShapes(luaWrap_lua_State* L)
{
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_number, 1))
    {
        ...
        if (res >= 0)
        {
            ...
            getInt32ArrayFromTable(L, 1, tableSize, theTable);
            if (mergeShapes)
                tableSize = -tableSize;
...
int _simUngroupShape(luaWrap_lua_State* L)
{
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_number, 0))
    {
        ...
        if (handles != nullptr)
        {
            pushInt32ArrayAsTable(L, count, handles);
            delete[] handles;
            LUA_END(1);
        }
    }
...
int _simCreateTexture(luaWrap_lua_State* L)
{
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_string, 0, lua_arg_integer | lua_arg_optional, 0, lua_arg_number | lua_arg_optional, 2, lua_arg_number | lua_arg_optional, 2, lua_arg_number | lua_arg_optional, 3, lua_arg_integer | lua_arg_optional, 0, lua_arg_integer | lua_arg_optional, 2))
    {
        ...
        if (shapeHandle >= 0)
        {
            ...
            pushInt32ArrayAsTable(L, 2, resolution.data());
            LUA_END(3);
        }
    }
...
int _simGetShapeGeomInfo(luaWrap_lua_State* L)
{
    ...
    pushDoubleArrayAsTable(L, 4, floatData);
    LUA_END(3);
}
...
int _simSetShapeTexture(luaWrap_lua_State* L)
{
                            ...
                            lua_arg_number, 2))
    {
        ...
        int options = luaToInt(L, 4);
        double uvScaling[2];
        getDoubleArrayFromTable(L, 5, 2, uvScaling);
        double* posP = nullptr;
        double* orP = nullptr;
        double _pos[3];
        double _or[3];
        ...
        if (res >= 0)
        {
            if (res == 2)
            {
                getDoubleArrayFromTable(L, 6, 3, _pos);
                posP = _pos;
            }
            ...
            if (res >= 0)
            {
                if (res == 2)
                {
                    getDoubleArrayFromTable(L, 7, 3, _or);
                    orP = _or;
                }
    ...
    if (itScrObj->getScriptType() == sim_scripttype_main)
    {
        if (checkInputArguments(L, &errorString, argOffset, lua_arg_number, 0))
        {
            ...
            if (editMode == NO_EDIT_MODE)
                retVal = App::scenes->addOnScriptContainer->callScripts(callType, nullptr, nullptr);
        }
    }
    else
        errorString = SIM_ERROR_CAN_ONLY_BE_CALLED_FROM_MAIN_SCRIPT;
...
int _simAlignShapeBB(luaWrap_lua_State* L)
{
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_integer, 0, lua_arg_number, 7))
    {
        int shapeHandle = luaToInt(L, 1);
        double pose[7];
        getDoubleArrayFromTable(L, 2, 7, pose);
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_number, 0, lua_arg_number, 0))
    {
        ...
        { // provided data is in tables
            if (checkInputArguments(L, &errorString, argOffset, lua_arg_number, 0, lua_arg_number, 0, lua_arg_number, 3))
            {
                ...
                if (res >= 0)
                {
                    getDoubleArrayFromTable(L, 3, ptCnt * 3, &pts[0]);
                    if (res == 2)
                    {
                        _cols.resize(v);
                        getUInt8ArrayFromTable(L, 4, v, &_cols[0]);
                        cols = &_cols[0];
                    }
                    ...
                    if (res >= 0)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                _tags.resize(v / 3);
                                getUInt32ArrayFromTable(L, 5, v / 3, &_tags[0]);
                                tags = &_tags[0];
                            }
                            retVal = CALL_C_API(simInsertVoxelsIntoOctree, handle, options, &pts[0], ptCnt, cols, tags,
                                                                        nullptr);
...
int _simGetOctreeVoxels(luaWrap_lua_State* L)
{
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_number, 0))
    {
        ...
        if (ptCnt >= 0)
        {
            pushDoubleArrayAsTable(L, ptCnt * 3, p);
            LUA_END(1);
        }
    }
...
int _simInsertObjectIntoOctree(luaWrap_lua_State* L)
{
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_number, 0, lua_arg_number, 0, lua_arg_number, 0))
    {
        ...
        if (res >= 0)
        {
            if (res == 2)
            {
                getUInt8ArrayFromTable(L, 4, 3, col);
                c = col;
            }
            res = checkOneGeneralInputArgument(L, 5, lua_arg_number, 0, true, true, &errorString, argOffset);
            if (res == 2)
...
int _simInsertObjectIntoPointCloud(luaWrap_lua_State* L)
{
                            ...
                            0))
    {
        ...
        if (res >= 0)
        {
            if (res == 2)
            {
                getUInt8ArrayFromTable(L, 5, 3, col);
                c = col;
            }
...
int _simCheckOctreePointOccupancy(luaWrap_lua_State* L)
{
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_number, 0, lua_arg_number, 0, lua_arg_number, 3))
    {
        ...
        getDoubleArrayFromTable(L, 3, ptCnt * 3, &points[0]);
        unsigned int tag = 0;
        uint64_t location = 0;
...
int _simAuxFunc(luaWrap_lua_State* L)
{
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_string, 0))
    {
        ...
        if (cmd.compare("createMirror") == 0)
        {
            ...
            {
                double s[2];
                getDoubleArrayFromTable(L, 2, 2, s);
                CMirror* m = new CMirror();
                m->setMirrorWidth(s[0]);
                m->setMirrorHeight(s[1]);
                App::scene->sceneObjects->addObjectToScene(m, false, true);
        ...
        if (cmd.compare("setAssemblyMatchValues") == 0)
        {
                                    ...
                                    lua_arg_string, 0))
            {
                ...
                CSceneObject* it = App::scene->sceneObjects->getObjectFromHandle(objHandle);
                if (it != nullptr)
                {
        ...
        if (cmd.compare("drawImageLines") == 0)
        {
            ...
            {
                int res[2];
                getInt32ArrayFromTable(L, 3, 2, res);
                size_t imgLen;
                ...
                if (imgLen == size_t(3 * res[0] * res[1]))
                {
                    ...
                    vc = 2 * (vc / 2);
                    lines.resize(vc);
                    getInt32ArrayFromTable(L, 4, vc, &lines[0]);
                    int col[3];
                    getInt32ArrayFromTable(L, 5, 3, col);
        ...
        if (cmd.compare("drawImageSquares") == 0)
        {
                                    ...
                                    lua_arg_number, 2, lua_arg_number, 3, lua_arg_number, 0))
            {
                ...
                if (imgLen == size_t(3 * res[0] * res[1]))
                {
                    ...
                    vc = 4 * (vc / 4);
                    rects.resize(vc);
                    getInt32ArrayFromTable(L, 4, vc, &rects[0]);
                    int col[3];
                    getInt32ArrayFromTable(L, 5, 3, col);
                    int lineWidth = luaToInt(L, 6);
                    QImage qimg((const unsigned char*)img, res[0], res[1], res[0] * 3, QImage::Format_RGB888);
        ...
        if (luaWrap_lua_isnonbuffertable(L, 2))
        {
            ...
            if (cnt > 0)
            {
                if (checkInputArguments(L, &errorString, argOffset, lua_arg_number, 0, lua_arg_integer, cnt))
                {
                    std::vector<int> handles;
                    handles.resize(cnt);
                    getInt32ArrayFromTable(L, 2, cnt, &handles[0]);
...
int _simGetReferencedHandles(luaWrap_lua_State* L)
{
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_number, 0))
    {
        ...
        if (cnt >= 0)
        {
            pushInt32ArrayAsTable(L, cnt, handles);
            delete[] handles;
            LUA_END(1);
        }
    }
...
int _simGetGraphCurve(luaWrap_lua_State* L)
{
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_number, 0, lua_arg_number, 0, lua_arg_number, 0))
    {
        ...
        int index = luaToInt(L, 3);
        CGraph* graph = App::scene->sceneObjects->getGraphFromHandle(graphHandle);
        if (graph != nullptr)
        {
            ...
            if (graph->getGraphCurveData(graphType, index, label, xVals, yVals, curveType, col, minMax, curveId, curveWidth, curveUid))
            {
                ...
                pushFloatArrayAsTable(L, 3, col);
                if (xVals.size() > 0)
                    pushDoubleArrayAsTable(L, (int)xVals.size(), &xVals[0]);
                else
                    pushInt32ArrayAsTable(L, 0, nullptr); // empty table
                if (yVals.size() > 0)
                    pushDoubleArrayAsTable(L, (int)yVals.size(), &yVals[0]);
                else
                    pushInt32ArrayAsTable(L, 0, nullptr); // empty table
                pushDoubleArrayAsTable(L, 6, minMax);
...
int _simGetGraphInfo(luaWrap_lua_State* L)
{
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_number, 0))
    {
        ...
        if (graph != nullptr)
        {
            ...
            pushFloatArrayAsTable(L, 3, graph->backgroundColor);
            pushFloatArrayAsTable(L, 3, graph->foregroundColor);
            LUA_END(3);
        }
        else
            errorString = SIM_ERROR_OBJECT_NOT_GRAPH;
...
int _simGetApiFunc(luaWrap_lua_State* L)
{
    ...
    if (checkInputArguments(L, &errorString, argOffset, lua_arg_number, 0, lua_arg_string, 0))
    {
        ...
        pushStringArrayAsTable(L, strTable);
        LUA_END(1);
    }
