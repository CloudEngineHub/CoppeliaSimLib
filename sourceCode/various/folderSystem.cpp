...
void CFolderSystem::setScenesPath(const char* path)
{
    ...
    if (diff)
    {
        ...
        {
            const char* cmd = prop(PropApp::sceneDir).name;
...
void CFolderSystem::setModelsPath(const char* path)
{
    ...
    if (diff)
    {
        ...
        if ((App::scenes != nullptr) && App::scenes->getEventsEnabled())
        {
            const char* cmd = prop(PropApp::modelDir).name;
            CCbor* ev = App::scenes->createObjectChangedEvent(sim_handle_app, cmd, true);
...
void CFolderSystem::setImportExportPath(const char* path)
{
    ...
    if (diff)
    {
        ...
        if ((App::scenes != nullptr) && App::scenes->getEventsEnabled())
        {
            const char* cmd = prop(PropApp::importExportDir).name;
            CCbor* ev = App::scenes->createObjectChangedEvent(sim_handle_app, cmd, true);
...
{
    std::string folder =
        _tempDataPath + "/sceneData" + std::to_string(App::scene->environment->getSceneUniqueID());
