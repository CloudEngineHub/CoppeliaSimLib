...
CSceneObject::~CSceneObject()
{
    delete _sceneObjectCustomizationPart;
...
void CSceneObject::_setMeasuredVelocity(const C3Vector& lin, const C3Vector& ang, const C3Vector& rotAxis, double angle)
{
    ...
    if (diff)
    {
        ...
        {
            const char* cmd = prop(PropSceneObject::calcLinearVelocity).name;
...
void CSceneObject::setHierarchyColorIndex(int c)
{
    ...
    if (diff)
    {
        ...
        if (_isInScene && App::scenes->getEventsEnabled())
        {
            const char* cmd = prop(PropSceneObject::hierarchyColor).name;
            CCbor* ev = App::scenes->createSceneObjectChangedEvent(this, false, cmd, true);
...
void CSceneObject::setCollectionSelfCollisionIndicator(int c)
{
    ...
    if (diff)
    {
        ...
        if (_isInScene && App::scenes->getEventsEnabled())
        {
            const char* cmd = prop(PropSceneObject::collectionSelfCollInd).name;
            CCbor* ev = App::scenes->createSceneObjectChangedEvent(this, false, cmd, true);
...
void CSceneObject::setDynamicFlag(int flag)
{
    ...
    if (diff)
    {
        ...
        if (_isInScene && App::scenes->getEventsEnabled())
        {
            const char* cmd = prop(PropSceneObject::dynamicFlag).name;
            CCbor* ev = App::scenes->createSceneObjectChangedEvent(this, true, cmd, true);
...
{
    return _sceneObjectCustomizationPart;
}
            ...
            else
            {
                if (it->getParent() == nullptr)
                {
                    if (it->getModelBase() || doNotSkip ||
                        (App::scene->sceneObjects->getObjectFromPath(nullptr, retVal.c_str(), 0) != this))
                    {
                ...
                else
                {
                    ...
                    tmp += retVal;
                    if (it->getModelBase() || doNotSkip ||
                        (App::scene->sceneObjects->getObjectFromPath(it->getParent(), tmp.c_str(), 0) != this))
                    {
                        previousAlias = itAlias;
                        retVal = "/" + previousAlias + retVal;
                        cnt++;
...
std::string CSceneObject::getObjectPathAndIndex(size_t modelCnt) const
{
    ...
    for (int i = int(objects.size()) - 1; i >= 0; i--)
    {
        ...
        while (true)
        {
            if (it == App::scene->sceneObjects->getObjectFromPath(nullptr, retVal.c_str(), index))
                break;
            index++;
        }
        // Following: we try to omit {0} if {1} doesn't exist
        if ((index != 0) || (App::scene->sceneObjects->getObjectFromPath(nullptr, retVal.c_str(), 1) != nullptr))
...
{ // to be only called from object container!
    ...
    if (diff)
    {
        ...
        if (_isInScene && App::scenes->getEventsEnabled())
        {
            const char* cmd = prop(PropSceneObject::selected).name;
            CCbor* ev = App::scenes->createSceneObjectChangedEvent(this, false, cmd, true);
...
void CSceneObject::_setModelInvisible(bool inv)
{
    ...
    if (diff)
    {
        ...
        if (_isInScene && App::scenes->getEventsEnabled())
        {
            const char* cmd = prop(PropSceneObject::modelInvisible).name;
            CCbor* ev = App::scenes->createSceneObjectChangedEvent(this, true, cmd, true);
    ...
    { // ok, no pickpass and select invisible
        ...
        else
        {
            display = ((!isObjectPartOfInvisibleModel()) &&
                       ((App::scene->environment->getActiveLayers() & getVisibilityLayer()) ||
            ...
            if (display)
            {
                ...
                else
                {
                    ...
                    {
                        if (_authorizedViewableObjects <= sim_object_sceneobjectend)
                            display = (_authorizedViewableObjects == viewableHandle);
                        else
                        {
                            CCollection* gr =
                                App::scene->collections->getObjectFromHandle(_authorizedViewableObjects);
                            if (gr != nullptr)
...
void CSceneObject::setModelAcknowledgement(const char* a)
{
    ...
    if (diff)
    {
        ...
        if (_isInScene && App::scenes->getEventsEnabled())
        {
            const char* cmd = prop(PropSceneObject::modelAcknowledgment).name;
            CCbor* ev = App::scenes->createSceneObjectChangedEvent(this, false, cmd, true);
...
{ // is also called from the ungroup/divide shape routines!!
    ...
    if (diff)
    {
        ...
        if (_isInScene && App::scenes->getEventsEnabled())
        {
            const char* cmd = prop(PropSceneObject::modelBase).name;
            CCbor* ev = App::scenes->createSceneObjectChangedEvent(this, true, cmd, true);
...
void CSceneObject::setObjectProperty(int p)
{
    ...
    if (diff)
    {
        ...
        if (_isInScene && App::scenes->getEventsEnabled())
        {
            const char* cmd = prop(PropSceneObject::objectProperty).name;
            CCbor* ev = App::scenes->createSceneObjectChangedEvent(this, true, cmd, true);
...
void CSceneObject::setLocalObjectSpecialProperty(int prope)
{ // sets the local value
    prope &= _getAllowedObjectSpecialProperties();
    bool diff = (_localObjectSpecialProperty != prope);
    if (diff)
    {
        int cb = _localObjectSpecialProperty ^ prope;
        _localObjectSpecialProperty = prope;
        if (_isInScene && App::scenes->getEventsEnabled())
        {
            if (cb & (sim_objectspecialproperty_collidable | sim_objectspecialproperty_measurable | sim_objectspecialproperty_detectable))
            {
                const char* cmd = prop(PropSceneObject::collidable).name;
                CCbor* ev = App::scenes->createSceneObjectChangedEvent(this, false, cmd, true);
...
bool CSceneObject::setModelProperty(int prope)
{ // model properties are actually override properties
    bool wm = true;
    if (prope & sim_modelproperty_not_model)
    {
        prope = sim_modelproperty_not_model;
        wm = false;
    }
    if (wm != _modelBase)
        setModelBase(wm);
    bool diff = (_modelProperty != prope);
    if (diff)
    {
        int cb = _modelProperty ^ prope;
        _modelProperty = prope;
        if (_isInScene && App::scenes->getEventsEnabled())
        {
            const char* cmd = prop(PropSceneObject::modelProperty).name;
            CCbor* ev = App::scenes->createSceneObjectChangedEvent(this, true, cmd, true);
            if (App::getEventProtocolVersion() == 2)
...
{
    return ((App::scene->environment->getActiveLayers() & _visibilityLayer) != 0);
}
...
void CSceneObject::scaleObject(double scalingFactor)
{
    ...
    App::scenes->setModificationFlag(256); // object scaled
    computeBoundingBox();
...
void CSceneObject::addObjectEventData(CCbor* ev, bool sendAsChildlessOrphanMeshless /*= false*/)
{
    ...
    if (App::getEventProtocolVersion() <= 3)
    {
        ev->appendKeyInt32Array(prop(PropSceneObject::children).name, ch.data(), ch.size());
        ...
        ev->appendKeyDoubleArray(prop(PropSceneObject::position).name, p, 3);
        ev->appendKeyDoubleArray(prop(PropSceneObject::pose).name, p, 7);
        ev->appendKeyDoubleArray(prop(PropSceneObject::quaternion).name, p + 3, 4);
    }
    else
    {
    ...
    if (App::getEventProtocolVersion() <= 3)
        ev->appendKeyText("alias", _objectAlias.c_str());
    else
    ...
    if (App::getEventProtocolVersion() <= 3)
        ev->appendKeyInt64(prop(PropSceneObject::parent).name, pH);
    else
    ...
    if (App::getEventProtocolVersion() <= 3)
    {
        double p[7];
        _bbFrame.getData(p, true);
        ev->appendKeyDoubleArray(prop(PropSceneObject::bbPose).name, p, 7);
        ev->appendKeyDoubleArray(prop(PropSceneObject::DEPRECATED_bbHsize).name, _bbHalfSize.data, 3);
    }
    else
    {
    ...
    /*
    ev->appendKeyBool(prop(PropSceneObject::movTranslX).name, (_objectMovementOptions & 64) == 0);
    ev->appendKeyBool(prop(PropSceneObject::movTranslY).name, (_objectMovementOptions & 128) == 0);
    ev->appendKeyBool(prop(PropSceneObject::movTranslZ).name, (_objectMovementOptions & 256) == 0);
    ev->appendKeyBool(prop(PropSceneObject::movRotX).name, (_objectMovementOptions & 512) == 0);
    ev->appendKeyBool(prop(PropSceneObject::movRotY).name, (_objectMovementOptions & 1024) == 0);
    ev->appendKeyBool(prop(PropSceneObject::movRotZ).name, (_objectMovementOptions & 2048) == 0);
    */
...
{ // This routine should be called in the very first line of function
    ...
    if (_sceneObjectCustomizationPart != nullptr)
        theNewObject->_sceneObjectCustomizationPart = _sceneObjectCustomizationPart->copyYourself(theNewObject);
...
void CSceneObject::writeCustomDataBlock(bool tmpData, const char* dataName, const char* data, size_t dataLength)
{
    ...
    if (diff && (!tmpData) && _isInScene && App::scenes->getEventsEnabled())
    {
        CCbor* ev = App::scenes->createSceneObjectChangedEvent(this, true, nullptr, false);
...
{ // bits 0-2: position x,y,z, bits 3-5: Euler e9,e1,e2
    ...
    if (diff)
    {
        ...
        if (_isInScene && App::scenes->getEventsEnabled())
        {
            const char* cmd = prop(PropSceneObject::movementPreferredAxes).name;
            CCbor* ev = App::scenes->createSceneObjectChangedEvent(this, true, cmd, true);
            if (App::getEventProtocolVersion() == 2)
...
void CSceneObject::setObjectMovementOptions(int p)
{
    ...
    if (diff)
    {
        ...
        if (_isInScene && App::scenes->getEventsEnabled())
        {
            const char* cmd = prop(PropSceneObject::movementOptions).name;
            CCbor* ev = App::scenes->createSceneObjectChangedEvent(this, true, cmd, true);
            if (App::getEventProtocolVersion() == 2)
...
void CSceneObject::setObjectMovementRelativity(int index, int p)
{
    ...
    if (diff)
    {
        ...
        if (_isInScene && App::scenes->getEventsEnabled())
        {
            const char* cmd = prop(PropSceneObject::movementRelativity).name;
            CCbor* ev = App::scenes->createSceneObjectChangedEvent(this, true, cmd, true);
...
void CSceneObject::setObjectMovementStepSize(int index, double s)
{
    ...
    if (diff)
    {
        ...
        if (_isInScene && App::scenes->getEventsEnabled())
        {
            const char* cmd = prop(PropSceneObject::movementStepSize).name;
            CCbor* ev = App::scenes->createSceneObjectChangedEvent(this, true, cmd, true);
...
{ // called before simulationEnded
    if (_initialValuesInitialized && _initialConfigurationMemorized)
    {
        if ((getCumulativeModelProperty() & sim_modelproperty_not_reset) == 0)
        {
            ...
            {
                CSceneObject* oldParent = App::scene->sceneObjects->getObjectFromUid(_initialParentUniqueId);
                App::scene->sceneObjects->setObjectParent(this, oldParent, true);
            }
        }
    }
}
...
void CSceneObject::setDynamicSimulationIconCode(int c)
{
    ...
    if (diff)
    {
        ...
        if (_isInScene && App::scenes->getEventsEnabled())
        {
            const char* cmd = prop(PropSceneObject::dynamicIcon).name;
            CCbor* ev = App::scenes->createSceneObjectChangedEvent(this, false, cmd, true);
...
void CSceneObject::generateDnaString()
{
    ...
    if (_isInScene && App::scenes->getEventsEnabled())
    {
        const char* cmd = prop(PropSceneObject::dna).name;
        CCbor* ev = App::scenes->createSceneObjectChangedEvent(this, false, cmd, true);
...
void CSceneObject::_setLocalTransformation_send(const CPose& tr) const
{
    ...
    if (_ikPluginCounterpartHandle != -1)
        App::scenes->pluginContainer->oldIkPlugin_setObjectLocalTransformation(_ikPluginCounterpartHandle,
...
void CSceneObject::_setParent_send(int parentHandle) const
{
    ...
    if (_ikPluginCounterpartHandle != -1)
    {
        ...
        App::scenes->pluginContainer->oldIkPlugin_setObjectParent(_ikPluginCounterpartHandle, p);
    }
}
...
bool CSceneObject::setParent(CSceneObject* parent)
{
    ...
    if (diff)
    {
        ...
        if (_isInScene && App::scenes->getEventsEnabled())
        {
            const char* cmd = prop(PropSceneObject::parentUid).name;
            CCbor* ev = App::scenes->createSceneObjectChangedEvent(this, true, cmd, true);
            ...
            if (App::getEventProtocolVersion() <= 3)
                ...
                ev->appendKeyHandle(prop(PropSceneObject::parent).name, pH);
            ev->appendKeyInt64(prop(PropSceneObject::DEPRECATED_parentHandle).name, pH); // for backward compatibility
            App::scenes->pushEvent();
        }
        _setParent_send(pH);
    }
    ...
    else
    {
        ...
        else
        {
            ...
            if (ar.xmlPushChildNode("common", exhaustiveXml))
            {
                ...
                std::string tmp;
                if (ar.xmlGetNode_string("metaInfo", tmp, false))
                    setMetaInfo(tmp.c_str());
...
void CSceneObject::performObjectLoadingMapping(const std::map<int, int>* map, int opType)
{
    ...
    if (opType <= 1) // scene or model loading only
        App::scene->sceneObjects->setObjectParent(this, App::scene->sceneObjects->getObjectFromHandle(_parentObjectHandle_forSerializationOnly), false);
    ...
    else
    {
        ...
        if (cnt != 0)
        {
            for (int i = 0; i < cnt; i++)
            {
                ...
                if (handles[i] >= 0)
                {
                    if (App::scene->sceneObjects->getObjectFromHandle(handles[i]) != nullptr)
                        r.generalObjectHandle = handles[i];
                    else
                    { // backw. compatibility:
                        if (App::scene->collisions_old->getObjectFromHandle(handles[i]) != nullptr)
                        {
                            r.generalObjectType = sim_appobj_collision_type;
                            r.generalObjectHandle = handles[i];
                        }
                        if (App::scene->distances_old->getObjectFromHandle(handles[i]) != nullptr)
                        {
                            r.generalObjectType = sim_appobj_distance_type;
                            r.generalObjectHandle = handles[i];
                        }
                        if (App::scene->ikGroups_old->getObjectFromHandle(handles[i]) != nullptr)
                        {
                            r.generalObjectType = sim_appobj_ik_type;
                            r.generalObjectHandle = handles[i];
                        }
                        if (App::scene->collections->getObjectFromHandle(handles[i]) != nullptr)
                        {
                ...
                if (handles[i] >= 0)
                {
                    if (App::scene->sceneObjects->getObjectFromHandle(handles[i]) != nullptr)
                    {
                        ...
                        r.uniquePersistentIdString =
                            App::scene->sceneObjects->getObjectFromHandle(handles[i])->getUniquePersistentIdString();
                    }
                    else
                    { // backw. compatibility
                        if (App::scene->collisions_old->getObjectFromHandle(handles[i]) != nullptr)
                        {
                            ...
                            r.generalObjectHandle = handles[i];
                            r.uniquePersistentIdString =
                                App::scene->collisions_old->getObjectFromHandle(handles[i])->getUniquePersistentIdString();
                        }
                        if (App::scene->distances_old->getObjectFromHandle(handles[i]) != nullptr)
                        {
                            r.generalObjectType = sim_appobj_distance_type;
                            r.generalObjectHandle = handles[i];
                            r.uniquePersistentIdString =
                                App::scene->distances_old->getObjectFromHandle(handles[i])->getUniquePersistentIdString();
                        }
                        if (App::scene->ikGroups_old->getObjectFromHandle(handles[i]) != nullptr)
                        {
                            r.generalObjectType = sim_appobj_ik_type;
                            r.generalObjectHandle = handles[i];
                            r.uniquePersistentIdString =
                                App::scene->ikGroups_old->getObjectFromHandle(handles[i])->getUniquePersistentIdString();
                        }
                        if (App::scene->collections->getObjectFromHandle(handles[i]) != nullptr)
                        {
                            r.generalObjectType = sim_objecttype_collection;
                            r.generalObjectHandle = handles[i];
                            r.uniquePersistentIdString =
                                App::scene->collections->getObjectFromHandle(handles[i])->getUniquePersistentIdString();
                        }
                    }
                }
                it->second.push_back(r);
...
{ // bits 0-2: position x,y,z (relative to parent frame), bits 3-5: Euler e9,e1,e2 (relative to own frame)
    if ((!App::scene->simulation->isSimulationStopped()) && (getObjectMovementOptions() & 8))
    {
        _objectManipulationMode_flaggedForGridOverlay = 0;
        _objectManipulationModeEventId = -1;
        return (false);
    }
    if (App::scene->simulation->isSimulationStopped() && (getObjectMovementOptions() & 4))
    {
        _objectManipulationMode_flaggedForGridOverlay = 0;
        _objectManipulationModeEventId = -1;
        return (false);
...
{ // bits 0-2: position x,y,z (relative to parent frame), bits 3-5: Euler e9,e1,e2 (relative to own frame)
    ...
    if ((!App::scene->simulation->isSimulationStopped()) && (getObjectMovementOptions() & 2))
    {
        _objectManipulationMode_flaggedForGridOverlay = 0;
        _objectManipulationModeEventId = -1;
        return (false);
    }
    if (App::scene->simulation->isSimulationStopped() && (getObjectMovementOptions() & 1))
    {
        _objectManipulationMode_flaggedForGridOverlay = 0;
        _objectManipulationModeEventId = -1;
        return (false);
...
void CSceneObject::buildOrUpdate_oldIk()
{
    ...
    if (_ikPluginCounterpartHandle == -1)
        _ikPluginCounterpartHandle = App::scenes->pluginContainer->oldIkPlugin_createDummy();
...
{
    if (_ikPluginCounterpartHandle != -1)
        App::scenes->pluginContainer->oldIkPlugin_eraseObject(_ikPluginCounterpartHandle);
    _ikPluginCounterpartHandle = -1;
}
...
void CSceneObject::setVisibilityLayer(int l)
{
    ...
    if (diff)
    {
        ...
        if (_isInScene && App::scenes->getEventsEnabled())
        {
            const char* cmd = prop(PropSceneObject::layer).name;
            CCbor* ev = App::scenes->createSceneObjectChangedEvent(this, true, cmd, true);
...
void CSceneObject::setObjectAlias_direct(const char* newName)
{
    ...
    if (diff)
    {
        ...
        if (_isInScene && App::scenes->getEventsEnabled())
        {
            std::string cmd = prop(PropSceneObject::name).name;
            if (App::getEventProtocolVersion() <= 3)
                cmd = "alias";
            CCbor* ev = App::scenes->createSceneObjectChangedEvent(this, true, cmd.c_str(), true);
...
void CSceneObject::setLocalTransformation(const CPose& tr)
{
    ...
    if (diff)
    {
        ...
        if (_isInScene && App::scenes->getEventsEnabled())
        {
            const char* cmd = prop(PropSceneObject::pose).name;
            CCbor* ev = App::scenes->createSceneObjectChangedEvent(this, true, cmd, true);
            if (App::getEventProtocolVersion() <= 3)
            {
            ...
            {
                ev->appendKeyVector3(prop(PropSceneObject::position).name, tr.X);
                ev->appendKeyPose(cmd, tr);
                ev->appendKeyQuaternion(prop(PropSceneObject::quaternion).name, tr.Q);
            }
            App::scenes->pushEvent();
        }
        _setLocalTransformation_send(_localTransformation);
...
bool CSceneObject::_setChildren(std::vector<CSceneObject*>* children)
{
    ...
    if (diff)
    {
        ...
        if (_isInScene && App::scenes->getEventsEnabled())
        {
            const char* cmd = prop(PropSceneObject::children).name;
            CCbor* ev = App::scenes->createSceneObjectChangedEvent(this, false, cmd, true);
            std::vector<int> ch;
            for (size_t i = 0; i < _childList.size(); i++)
                ch.push_back(_childList[i]->getObjectHandle());
...
int CSceneObject::setBoolProperty(const char* ppName, bool pState)
{
    ...
    if (retVal == sim_propertyret_unknownproperty)
    {
        ...
        else if (strcmp(ppName, prop(PropSceneObject::hideFromModelBB).name) == 0)
        {
            retVal = sim_propertyret_ok;
        ...
        else if (_pName == prop(PropSceneObject::measurable).name)
        {
            retVal = sim_propertyret_ok;
        ...
        else if (_pName == prop(PropSceneObject::detectable).name)
        {
            retVal = sim_propertyret_ok;
...
int CSceneObject::getBoolProperty(const char* ppName, bool& pState) const
{
    ...
    if ((retVal == sim_propertyret_unknownproperty) && (_sceneObjectCustomizationPart != nullptr))
        retVal = _sceneObjectCustomizationPart->getBoolProperty(ppName, pState);
    if (retVal == sim_propertyret_unknownproperty)
    {
        std::string _pName(ppName);
        if (strcmp(ppName, prop(PropSceneObject::modelInvisible).name) == 0)
        {
            retVal = sim_propertyret_ok;
            pState = _modelInvisible;
        }
        else if (strcmp(ppName, prop(PropSceneObject::modelBase).name) == 0)
        {
            retVal = sim_propertyret_ok;
            pState = _modelBase;
        }
        ...
        else if (_pName == prop(PropSceneObject::visible).name)
        {
            retVal = sim_propertyret_ok;
            pState = isObjectVisible();
        }
        else if (_pName == prop(PropSceneObject::dynamicallyEnabled).name)
        {
            retVal = sim_propertyret_ok;
            pState = false;
...
{
    int retVal = Obj::setIntProperty(ppName, pState);
    if ((retVal == sim_propertyret_unknownproperty) && (_sceneObjectCustomizationPart != nullptr))
        retVal = _sceneObjectCustomizationPart->setIntProperty(ppName, pState);
    if (retVal == sim_propertyret_unknownproperty)
    {
        std::string _pName(ppName);
        if (strcmp(ppName, prop(PropSceneObject::layer).name) == 0)
        {
            retVal = sim_propertyret_ok;
            setVisibilityLayer(pState);
        }
        else if (strcmp(ppName, prop(PropSceneObject::DEPRECATED_parentHandle).name) == 0)
        {
            retVal = 0;
            CSceneObject* newParent = App::scene->sceneObjects->getObjectFromHandle(pState);
            if ( (newParent != nullptr) || (pState == -1) )
            {
                if (App::scene->sceneObjects->setObjectParent(this, newParent, false))
                    retVal = sim_propertyret_ok;
            }
        }
        else if (strcmp(ppName, prop(PropSceneObject::objectProperty).name) == 0)
        {
            retVal = sim_propertyret_ok;
            setObjectProperty(pState);
        }
        else if (strcmp(ppName, prop(PropSceneObject::modelProperty).name) == 0)
        {
            retVal = sim_propertyret_ok;
            setModelProperty(pState);
        }
        else if (strcmp(ppName, prop(PropSceneObject::movementOptions).name) == 0)
        {
            retVal = sim_propertyret_ok;
            setObjectMovementOptions(pState);
        }
        else if (strcmp(ppName, prop(PropSceneObject::movementPreferredAxes).name) == 0)
        {
            retVal = sim_propertyret_ok;
            setObjectMovementPreferredAxes(pState);
        }
        else if (_pName == prop(PropSceneObject::hierarchyColor).name)
        {
            retVal = sim_propertyret_ok;
            setHierarchyColorIndex(pState);
        }
        else if (_pName == prop(PropSceneObject::collectionSelfCollInd).name)
        {
            retVal = sim_propertyret_ok;
...
{
    int retVal = Obj::getIntProperty(ppName, pState);
    if ((retVal == sim_propertyret_unknownproperty) && (_sceneObjectCustomizationPart != nullptr))
        retVal = _sceneObjectCustomizationPart->getIntProperty(ppName, pState);
    if (retVal == sim_propertyret_unknownproperty)
    {
        std::string _pName(ppName);
        if (strcmp(ppName, prop(PropSceneObject::layer).name) == 0)
        ...
        else if (strcmp(ppName, prop(PropSceneObject::childOrder).name) == 0)
        {
            retVal = sim_propertyret_ok;
            pState = _childOrder;
        }
        ...
        else if (_pName == prop(PropSceneObject::dynamicIcon).name)
        {
            retVal = sim_propertyret_ok;
            pState = _dynamicSimulationIconCode;
        }
        else if (_pName == prop(PropSceneObject::dynamicFlag).name)
        {
            retVal = sim_propertyret_ok;
            pState = _dynamicFlag;
        }
...
int CSceneObject::setLongProperty(const char* ppName, int64_t pState)
{
    ...
    if ((retVal == sim_propertyret_unknownproperty) && (_sceneObjectCustomizationPart != nullptr))
        retVal = _sceneObjectCustomizationPart->setLongProperty(ppName, pState);
    if (retVal == sim_propertyret_unknownproperty)
    {
    }
    return retVal;
...
int CSceneObject::getLongProperty(const char* ppName, int64_t& pState) const
{
    ...
    if ((retVal == sim_propertyret_unknownproperty) && (_sceneObjectCustomizationPart != nullptr))
        retVal = _sceneObjectCustomizationPart->getLongProperty(ppName, pState);
    if (retVal == sim_propertyret_unknownproperty)
    {
        if (strcmp(ppName, prop(PropSceneObject::parentUid).name) == 0)
        {
            retVal = sim_propertyret_ok;
            pState = -1;
            if (_parentObject != nullptr)
        ...
        else if (strcmp(ppName, prop(PropSceneObject::uid).name) == 0)
        {
            retVal = sim_propertyret_ok;
            pState = _objectUid;
        }
...
int CSceneObject::setHandleProperty(const char* ppName, int64_t pState)
{
    ...
    if ((retVal == sim_propertyret_unknownproperty) && (_sceneObjectCustomizationPart != nullptr))
        retVal = _sceneObjectCustomizationPart->setHandleProperty(ppName, pState);
    if (retVal == sim_propertyret_unknownproperty)
    {
        std::string _pName(ppName);
        if (strcmp(ppName, prop(PropSceneObject::parent).name) == 0)
        {
            retVal = 0;
            CSceneObject* newParent = App::scene->sceneObjects->getObjectFromHandle(pState);
            if ( (newParent != nullptr) || (pState == -1) )
...
int CSceneObject::getHandleProperty(const char* ppName, int64_t& pState) const
{
    ...
    if ((retVal == sim_propertyret_unknownproperty) && (_sceneObjectCustomizationPart != nullptr))
        retVal = _sceneObjectCustomizationPart->getHandleProperty(ppName, pState);
    if (retVal == sim_propertyret_unknownproperty)
    {
        std::string _pName(ppName);
        if (strcmp(ppName, prop(PropSceneObject::parent).name) == 0)
...
{
    int retVal = Obj::setFloatProperty(ppName, pState);
    if ((retVal == sim_propertyret_unknownproperty) && (_sceneObjectCustomizationPart != nullptr))
        retVal = _sceneObjectCustomizationPart->setFloatProperty(ppName, pState);
    if (retVal == sim_propertyret_unknownproperty)
    {
    }
    return retVal;
...
{
    int retVal = Obj::getFloatProperty(ppName, pState);
    if ((retVal == sim_propertyret_unknownproperty) && (_sceneObjectCustomizationPart != nullptr))
        retVal = _sceneObjectCustomizationPart->getFloatProperty(ppName, pState);
    if (retVal == sim_propertyret_unknownproperty)
    {
        std::string _pName(ppName);
...
{
    int retVal = Obj::setStringProperty(ppName, pState);
    if ((retVal == sim_propertyret_unknownproperty) && (_sceneObjectCustomizationPart != nullptr))
        retVal = _sceneObjectCustomizationPart->setStringProperty(ppName, pState);
    if (retVal == sim_propertyret_unknownproperty)
    {
        std::string _pName(ppName);
        if (_pName == prop(PropSceneObject::name).name)
        {
            App::scene->sceneObjects->setObjectAlias(this, pState.c_str(), false);
            retVal = sim_propertyret_ok;
        }
        else if (_pName == prop(PropSceneObject::DEPRECATED_deprecatedName).name)
        {
            setObjectName_direct_old(pState.c_str());
            retVal = sim_propertyret_ok;
        }
        else if (_pName == prop(PropSceneObject::modelAcknowledgment).name)
        {
...
int CSceneObject::getStringProperty(const char* ppName, std::string& pState) const
{
    ...
    if ((retVal == sim_propertyret_unknownproperty) && (_sceneObjectCustomizationPart != nullptr))
        retVal = _sceneObjectCustomizationPart->getStringProperty(ppName, pState);
    if (retVal == sim_propertyret_unknownproperty)
    {
        std::string _pName(ppName);
        if (_pName == prop(PropSceneObject::name).name)
        ...
        else if (_pName == prop(PropSceneObject::persistentUid).name)
        {
            retVal = sim_propertyret_ok;
            pState = _uniquePersistentIdString;
        }
        else if (_pName == prop(PropSceneObject::modelHash).name)
        {
            retVal = sim_propertyret_ok;
            if (_isInScene)
            {
...
int CSceneObject::setBufferProperty(const char* ppName, const std::string& pState)
{
    ...
    if ((retVal == sim_propertyret_unknownproperty) && (_sceneObjectCustomizationPart != nullptr))
        retVal = _sceneObjectCustomizationPart->setBufferProperty(ppName, pState);
    if (retVal == sim_propertyret_unknownproperty)
    {
        std::string _pName(ppName);
...
int CSceneObject::getBufferProperty(const char* ppName, std::string& pState) const
{
    ...
    if ((retVal == sim_propertyret_unknownproperty) && (_sceneObjectCustomizationPart != nullptr))
        retVal = _sceneObjectCustomizationPart->getBufferProperty(ppName, pState);
    if (retVal == sim_propertyret_unknownproperty)
    {
        std::string _pName(ppName);
        std::string pN(ppName);
        ...
        if (retVal == sim_propertyret_unknownproperty)
        {
            if (_pName == prop(PropSceneObject::dna).name)
            {
                retVal = sim_propertyret_ok;
                pState = _dnaString;
            }
...
{
    int retVal = Obj::setIntArray2Property(ppName, pState);
    if ((retVal == sim_propertyret_unknownproperty) && (_sceneObjectCustomizationPart != nullptr))
        retVal = _sceneObjectCustomizationPart->setIntArray2Property(ppName, pState);
    if (retVal == sim_propertyret_unknownproperty)
    {
    }
    return retVal;
...
int CSceneObject::getIntArray2Property(const char* ppName, int* pState) const
{
    ...
    if ((retVal == sim_propertyret_unknownproperty) && (_sceneObjectCustomizationPart != nullptr))
        retVal = _sceneObjectCustomizationPart->getIntArray2Property(ppName, pState);
    if (retVal == sim_propertyret_unknownproperty)
    {
    }
    return retVal;
...
{
    int retVal = Obj::setVector3Property(ppName, pState);
    if ((retVal == sim_propertyret_unknownproperty) && (_sceneObjectCustomizationPart != nullptr))
        retVal = _sceneObjectCustomizationPart->setVector3Property(ppName, pState);
    if (retVal == sim_propertyret_unknownproperty)
    {
        std::string _pName(ppName);
        if (strcmp(ppName, prop(PropSceneObject::position).name) == 0)
        {
            retVal = sim_propertyret_ok;
            CPose tr(_localTransformation);
            tr.X = pState;
        ...
        else if (strcmp(ppName, prop(PropSceneObject::eulerAngles).name) == 0)
        {
            retVal = sim_propertyret_ok;
            CPose tr(_localTransformation);
        ...
        else if (strcmp(ppName, prop(PropSceneObject::absPosition).name) == 0)
        {
            retVal = sim_propertyret_ok;
            CPose tr(getCumulativeTransformation());
            tr.X = pState;
        ...
        else if (strcmp(ppName, prop(PropSceneObject::absEulerAngles).name) == 0)
        {
            retVal = sim_propertyret_ok;
            CPose tr(getCumulativeTransformation());
            tr.Q = CQuaternion(pState);
        ...
        else if (strcmp(ppName, prop(PropSceneObject::size).name) == 0)
        {
            retVal = sim_propertyret_ok;
...
{
    int retVal = Obj::getVector3Property(ppName, pState);
    if ((retVal == sim_propertyret_unknownproperty) && (_sceneObjectCustomizationPart != nullptr))
        retVal = _sceneObjectCustomizationPart->getVector3Property(ppName, pState);
    if (retVal == sim_propertyret_unknownproperty)
    {
        std::string _pName(ppName);
        if (_pName == prop(PropSceneObject::calcLinearVelocity).name)
        {
            retVal = sim_propertyret_ok;
        ...
        else if (_pName == prop(PropSceneObject::size).name)
        {
            retVal = sim_propertyret_ok;
            pState = _bbHalfSize * 2.0;
        }
        else if (_pName == prop(PropSceneObject::DEPRECATED_bbHsize).name)
        {
            retVal = sim_propertyret_ok;
            pState = _bbHalfSize;
        }
        else if ( (_pName == prop(PropSceneObject::modelBBSize).name) || (_pName == prop(PropSceneObject::modelBBPos).name) )
        {
            retVal = sim_propertyret_ok;
            ...
            {
                retVal = sim_propertyret_ok;
                if (_pName == prop(PropSceneObject::modelBBSize).name)
                    pState = maxV - minV;
                else
                    pState = (maxV + minV) * 0.5;
            }
        }
        else if (strcmp(ppName, prop(PropSceneObject::position).name) == 0)
        {
            retVal = sim_propertyret_ok;
...
{
    int retVal = Obj::setMatrixProperty(ppName, pState);
    if ((retVal == sim_propertyret_unknownproperty) && (_sceneObjectCustomizationPart != nullptr))
        retVal = _sceneObjectCustomizationPart->setMatrixProperty(ppName, pState);
    if (retVal == sim_propertyret_unknownproperty)
    {
    }
    return retVal;
...
{
    int retVal = Obj::getMatrixProperty(ppName, pState);
    if ((retVal == sim_propertyret_unknownproperty) && (_sceneObjectCustomizationPart != nullptr))
        retVal = _sceneObjectCustomizationPart->getMatrixProperty(ppName, pState);
    if (retVal == sim_propertyret_unknownproperty)
    {
    }
    return retVal;
...
int CSceneObject::setQuaternionProperty(const char* ppName, const CQuaternion& pState)
{
    ...
    if ((retVal == sim_propertyret_unknownproperty) && (_sceneObjectCustomizationPart != nullptr))
        retVal = _sceneObjectCustomizationPart->setQuaternionProperty(ppName, pState);
    if (retVal == sim_propertyret_unknownproperty)
    {
        std::string _pName(ppName);
        if (strcmp(ppName, prop(PropSceneObject::quaternion).name) == 0)
        {
            retVal = sim_propertyret_ok;
            CPose tr(_localTransformation);
            tr.Q = pState;
        ...
        else if (strcmp(ppName, prop(PropSceneObject::absQuaternion).name) == 0)
        {
            retVal = sim_propertyret_ok;
            CPose tr(getCumulativeTransformation());
            tr.Q = pState;
...
int CSceneObject::getQuaternionProperty(const char* ppName, CQuaternion& pState) const
{
    ...
    if ((retVal == sim_propertyret_unknownproperty) && (_sceneObjectCustomizationPart != nullptr))
        retVal = _sceneObjectCustomizationPart->getQuaternionProperty(ppName, pState);
    if (retVal == sim_propertyret_unknownproperty)
    {
        std::string _pName(ppName);
        if (strcmp(ppName, prop(PropSceneObject::quaternion).name) == 0)
...
int CSceneObject::setPoseProperty(const char* ppName, const CPose& pState)
{
    ...
    if ((retVal == sim_propertyret_unknownproperty) && (_sceneObjectCustomizationPart != nullptr))
        retVal = _sceneObjectCustomizationPart->setPoseProperty(ppName, pState);
    if (retVal == sim_propertyret_unknownproperty)
    {
        std::string _pName(ppName);
        if (strcmp(ppName, prop(PropSceneObject::pose).name) == 0)
        {
            retVal = sim_propertyret_ok;
            setLocalTransformation(pState);
        }
        else if (strcmp(ppName, prop(PropSceneObject::absPose).name) == 0)
        {
            retVal = sim_propertyret_ok;
...
int CSceneObject::getPoseProperty(const char* ppName, CPose& pState) const
{
    ...
    if ((retVal == sim_propertyret_unknownproperty) && (_sceneObjectCustomizationPart != nullptr))
        retVal = _sceneObjectCustomizationPart->getPoseProperty(ppName, pState);
    if (retVal == sim_propertyret_unknownproperty)
    {
        std::string _pName(ppName);
        if (strcmp(ppName, prop(PropSceneObject::pose).name) == 0)
        ...
        else if (strcmp(ppName, prop(PropSceneObject::bbPose).name) == 0)
        {
            retVal = sim_propertyret_ok;
            pState = _bbFrame;
        }
...
{
    int retVal = Obj::setColorProperty(ppName, pState);
    if ((retVal == sim_propertyret_unknownproperty) && (_sceneObjectCustomizationPart != nullptr))
        retVal = _sceneObjectCustomizationPart->setColorProperty(ppName, pState);
    if (retVal == sim_propertyret_unknownproperty)
    {
    }
    return retVal;
...
{
    int retVal = Obj::getColorProperty(ppName, pState);
    if ((retVal == sim_propertyret_unknownproperty) && (_sceneObjectCustomizationPart != nullptr))
        retVal = _sceneObjectCustomizationPart->getColorProperty(ppName, pState);
    if (retVal == sim_propertyret_unknownproperty)
    {
    }
    return retVal;
...
{
    int retVal = Obj::setFloatArrayProperty(ppName, pState);
    if ((retVal == sim_propertyret_unknownproperty) && (_sceneObjectCustomizationPart != nullptr))
        retVal = _sceneObjectCustomizationPart->setFloatArrayProperty(ppName, pState);
    if (retVal == sim_propertyret_unknownproperty)
    {
        std::string _pName(ppName);
        if (strcmp(ppName, prop(PropSceneObject::movementStepSize).name) == 0)
        {
            if (pState.size() >= 2)
            {
                retVal = sim_propertyret_ok;
...
{
    pState.clear();
    int retVal = Obj::getFloatArrayProperty(ppName, pState);
    if ((retVal == sim_propertyret_unknownproperty) && (_sceneObjectCustomizationPart != nullptr))
        retVal = _sceneObjectCustomizationPart->getFloatArrayProperty(ppName, pState);
    if (retVal == sim_propertyret_unknownproperty)
    {
        std::string _pName(ppName);
        if (strcmp(ppName, prop(PropSceneObject::movementStepSize).name) == 0)
...
{
    int retVal = Obj::setIntArrayProperty(ppName, pState);
    if ((retVal == sim_propertyret_unknownproperty) && (_sceneObjectCustomizationPart != nullptr))
        retVal = _sceneObjectCustomizationPart->setIntArrayProperty(ppName, pState);
    if (retVal == sim_propertyret_unknownproperty)
    {
        std::string _pName(ppName);
        if (strcmp(ppName, prop(PropSceneObject::movementRelativity).name) == 0)
        {
            if (pState.size() >= 2)
            {
                retVal = sim_propertyret_ok;
...
{
    pState.clear();
    int retVal = Obj::getIntArrayProperty(ppName, pState);
    if ((retVal == sim_propertyret_unknownproperty) && (_sceneObjectCustomizationPart != nullptr))
        retVal = _sceneObjectCustomizationPart->getIntArrayProperty(ppName, pState);
    if (retVal == sim_propertyret_unknownproperty)
    {
        std::string _pName(ppName);
        if (strcmp(ppName, prop(PropSceneObject::movementRelativity).name) == 0)
...
int CSceneObject::setHandleArrayProperty(const char* ppName, const std::vector<int64_t>& pState)
{
    ...
    if ((retVal == sim_propertyret_unknownproperty) && (_sceneObjectCustomizationPart != nullptr))
        retVal = _sceneObjectCustomizationPart->setHandleArrayProperty(ppName, pState);
    if (retVal == sim_propertyret_unknownproperty)
    {
        std::string _pName(ppName);
        std::string pN(ppName);
...
int CSceneObject::getHandleArrayProperty(const char* ppName, std::vector<int64_t>& pState) const
{
    ...
    if ((retVal == sim_propertyret_unknownproperty) && (_sceneObjectCustomizationPart != nullptr))
        retVal = _sceneObjectCustomizationPart->getHandleArrayProperty(ppName, pState);
    if (retVal == sim_propertyret_unknownproperty)
    {
        std::string _pName(ppName);
        if (strcmp(ppName, prop(PropSceneObject::children).name) == 0)
        {
            for (size_t i = 0; i < _childList.size(); i++)
                pState.push_back(_childList[i]->getObjectHandle());
            retVal = sim_propertyret_ok;
        }
        else if (strcmp(ppName, prop(PropSceneObject::tree).name) == 0)
        {
...
{
    int retVal = Obj::setStringArrayProperty(ppName, pState);
    if ((retVal == sim_propertyret_unknownproperty) && (_sceneObjectCustomizationPart != nullptr))
        retVal = _sceneObjectCustomizationPart->setStringArrayProperty(ppName, pState);
    if (retVal == sim_propertyret_unknownproperty)
    {
    }
    return retVal;
...
int CSceneObject::getStringArrayProperty(const char* ppName, std::vector<std::string>& pState) const
{
    ...
    if ((retVal == sim_propertyret_unknownproperty) && (_sceneObjectCustomizationPart != nullptr))
        retVal = _sceneObjectCustomizationPart->getStringArrayProperty(ppName, pState);
    if (retVal == sim_propertyret_unknownproperty)
    {
    }
    return retVal;
...
int CSceneObject::setMethodProperty(const char* ppName, const void* pState)
{
    int retVal = Obj::setMethodProperty(ppName, pState);
    if ((retVal == sim_propertyret_unknownproperty) && (_sceneObjectCustomizationPart != nullptr))
        retVal = _sceneObjectCustomizationPart->setMethodProperty(ppName, pState);
    if (retVal == sim_propertyret_unknownproperty)
    {
    }
    return retVal;
...
int CSceneObject::getMethodProperty(const char* ppName, void*& pState) const
{
    int retVal = Obj::getMethodProperty(ppName, pState);
    if ((retVal == sim_propertyret_unknownproperty) && (_sceneObjectCustomizationPart != nullptr))
        retVal = _sceneObjectCustomizationPart->getMethodProperty(ppName, pState);
    if (retVal == sim_propertyret_unknownproperty)
    {
    }
    return retVal;
...
int CSceneObject::setMethodProperty(const char* ppName, const std::string& pState)
{
    int retVal = Obj::setMethodProperty(ppName, pState);
    if ((retVal == sim_propertyret_unknownproperty) && (_sceneObjectCustomizationPart != nullptr))
        retVal = _sceneObjectCustomizationPart->setMethodProperty(ppName, pState);
...
int CSceneObject::getMethodProperty(const char* ppName, std::string& pState) const
{
    int retVal = Obj::getMethodProperty(ppName, pState);
    if ((retVal == sim_propertyret_unknownproperty) && (_sceneObjectCustomizationPart != nullptr))
        retVal = _sceneObjectCustomizationPart->getMethodProperty(ppName, pState);
...
{
    int retVal = Obj::removeProperty(ppName);
    if ((retVal == sim_propertyret_unknownproperty) && (_sceneObjectCustomizationPart != nullptr))
        retVal = _sceneObjectCustomizationPart->removeProperty(ppName);
    if (retVal == sim_propertyret_unknownproperty)
    {
        std::string _pName(ppName);
        std::string pN(ppName);
...
int CSceneObject::getPropertyInfo(const char* ppName, int& info, std::string& infoTxt) const
{
    ...
    if ((retVal == sim_propertyret_unknownproperty) && (_sceneObjectCustomizationPart != nullptr))
        retVal = _sceneObjectCustomizationPart->getPropertyInfo(ppName, info, infoTxt);
    if (retVal == sim_propertyret_unknownproperty)
    {
        for (size_t i = 0; i < allProps_sceneObject.size(); i++)
        {
            if (strcmp(allProps_sceneObject[i].name, ppName) == 0)
            {
                ...
                if (infoTxt == "j")
                    infoTxt = allProps_sceneObject[i].info.json;
                else
                {
...
{
    int retVal = Obj::setPropertyInfo(pName, info, infoTxt);
    if ((retVal == sim_propertyret_unknownproperty) && (_sceneObjectCustomizationPart != nullptr))
        retVal = _sceneObjectCustomizationPart->setPropertyInfo(pName, info, infoTxt);
    return retVal;
}
