...
void CCamera::performObjectLoadingMapping(const std::map<int, int>* map, int opType)
{
    ...
    _trackedObjectHandle = CScene::getLoadingMapping(map, _trackedObjectHandle);
}
...
int CCamera::getStringProperty(const char* ppName, std::string& pState) const
{
    std::string _pName(ppName);
...
{
    appartenance = _originalObjectTypeStr; // important here too, since viewableBase items are part of camera
