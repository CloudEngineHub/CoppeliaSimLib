...
void CSimulation::setRemoveNewObjectsAtSimulationEnd(bool r)
{
    ...
    if (diff)
    {
        ...
        {
            CCbor* ev = App::scenes->createObjectChangedEvent(sim_handle_scene, prop(PropScene::removeNewObjectsAtEnd).name, true);
            ev->appendKeyBool(prop(PropScene::removeNewObjectsAtEnd).name, _removeNewObjectsAtSimulationEnd);
...
void CSimulation::startOrResumeSimulation()
{
...
#endif
        App::scenes->simulationAboutToStart();
    ...
    {
        App::scenes->simulationAboutToResume();
        _realTimeCorrection = 0.0;
        setSimulationState(sim_simulation_running);
        simulationTime_real_lastInMs = (int)VDateTime::getTimeInMs();
...
void CSimulation::advanceSimulationByOneStep()
{
    ...
    if (isSimulationRunning())
    {
        ...
        if (_requestToPause)
        {
            ...
            App::scenes->simulationPaused(); // calls sim_syscb_suspend
            _requestToPause = false;
        }
        else
        {
            App::scenes->simulationAboutToStep();
            ...
            {
                App::scenes->simulationAboutToEnd();
                setSimulationState(sim_simulation_stopped);
                App::scenes->simulationEnded(_removeNewObjectsAtSimulationEnd);
            }
...
void CSimulation::setTimeStep(double dt, bool alsoWhenRunning /*= false*/)
{
    if (isSimulationStopped() || alsoWhenRunning)
    {
        ...
        if (diff)
        {
            ...
            if (App::scenes->getEventsEnabled())
            {
                CCbor* ev = App::scenes->createObjectChangedEvent(sim_handle_scene, prop(PropScene::timeStep).name, true);
                ev->appendKeyDouble(prop(PropScene::timeStep).name, _simulationTimeStep);
                App::scenes->pushEvent();
            }
#ifdef SIM_WITH_GUI
...
void CSimulation::setIsRealTimeSimulation(bool realTime)
{
    if (isSimulationStopped())
    {
        ...
        if (diff)
        {
            ...
            if (App::scenes->getEventsEnabled())
            {
                CCbor* ev = App::scenes->createObjectChangedEvent(sim_handle_scene, prop(PropScene::realtimeSimulation).name, true);
                ev->appendKeyBool(prop(PropScene::realtimeSimulation).name, _realTimeSimulation);
                App::scenes->pushEvent();
            }
        }
    }
...
void CSimulation::setRealTimeCoeff(double coeff)
{
    ...
    if (diff)
    {
        ...
        if (App::scenes->getEventsEnabled())
        {
            CCbor* ev = App::scenes->createObjectChangedEvent(sim_handle_scene, prop(PropScene::realtimeModifier).name, true);
            ev->appendKeyDouble(prop(PropScene::realtimeModifier).name, _realTimeCoefficient);
            App::scenes->pushEvent();
        }
    }
}
...
void CSimulation::setPassesPerRendering(int n)
{
    if (isSimulationStopped())
    {
        ...
        if (diff)
        {
            ...
            if (App::scenes->getEventsEnabled())
            {
                CCbor* ev = App::scenes->createObjectChangedEvent(sim_handle_scene, prop(PropScene::stepsPerRendering).name, true);
                ev->appendKeyInt64(prop(PropScene::stepsPerRendering).name, _simulationPassesPerRendering);
                App::scenes->pushEvent();
            }
        }
    }
...
void CSimulation::setSimulationStepCount(int cnt)
{
    ...
    if (diff)
    {
        ...
        if (App::scenes->getEventsEnabled())
        {
            CCbor* ev = App::scenes->createObjectChangedEvent(sim_handle_scene, prop(PropScene::stepCount).name, true);
            ev->appendKeyInt64(prop(PropScene::stepCount).name, _simulationStepCount);
            App::scenes->pushEvent();
        }
    }
}
...
void CSimulation::appendGenesisData(CCbor* ev) const
{
    ev->appendKeyBool(prop(PropScene::removeNewObjectsAtEnd).name, _removeNewObjectsAtSimulationEnd);
    ev->appendKeyBool(prop(PropScene::realtimeSimulation).name, _realTimeSimulation);
    ev->appendKeyBool(prop(PropScene::pauseSimulationAtTime).name, _pauseAtSpecificTime);
    ev->appendKeyBool(prop(PropScene::pauseSimulationAtError).name, _pauseAtError);
    ev->appendKeyInt64(prop(PropScene::simulationState).name, _simulationState);
    ev->appendKeyInt64(prop(PropScene::stepCount).name, _simulationStepCount);
    ev->appendKeyInt64(prop(PropScene::stepsPerRendering).name, _simulationPassesPerRendering);
    ev->appendKeyInt64(prop(PropScene::speedModifier).name, _speedModifierCount);
    ev->appendKeyDouble(prop(PropScene::simulationTime).name, _simulationTime);
    ev->appendKeyDouble(prop(PropScene::timeStep).name, _simulationTimeStep);
    ev->appendKeyDouble(prop(PropScene::timeToPause).name, _simulationTimeToPause);
    ev->appendKeyDouble(prop(PropScene::realtimeModifier).name, _realTimeCoefficient);
}
...
void CSimulation::setSimulationState(int state)
{
    ...
    if (diff)
    {
        ...
        if (App::scenes->getEventsEnabled())
        {
            CCbor* ev = App::scenes->createObjectChangedEvent(sim_handle_scene, prop(PropScene::simulationState).name, true);
            ev->appendKeyInt64(prop(PropScene::simulationState).name, _simulationState);
            App::scenes->pushEvent();
        }
    }
...
void CSimulation::setPauseAtError(bool br)
{
    ...
    if (diff)
    {
        ...
        if (App::scenes->getEventsEnabled())
        {
            CCbor* ev = App::scenes->createObjectChangedEvent(sim_handle_scene, prop(PropScene::pauseSimulationAtError).name, true);
            ev->appendKeyBool(prop(PropScene::pauseSimulationAtError).name, _pauseAtError);
            App::scenes->pushEvent();
        }
    }
...
void CSimulation::setPauseTime(double time)
{
    ...
    if (diff)
    {
        ...
        if (App::scenes->getEventsEnabled())
        {
            CCbor* ev = App::scenes->createObjectChangedEvent(sim_handle_scene, prop(PropScene::timeToPause).name, true);
            ev->appendKeyDouble(prop(PropScene::timeToPause).name, _simulationTimeToPause);
            App::scenes->pushEvent();
        }
    }
...
void CSimulation::setPauseAtSpecificTime(bool e)
{
    ...
    if (diff)
    {
        ...
        if (App::scenes->getEventsEnabled())
        {
            CCbor* ev = App::scenes->createObjectChangedEvent(sim_handle_scene, prop(PropScene::pauseSimulationAtTime).name, true);
            ev->appendKeyBool(prop(PropScene::pauseSimulationAtTime).name, _pauseAtSpecificTime);
            App::scenes->pushEvent();
        }
    }
...
void CSimulation::_setSimulationTime(double t)
{
    ...
    if (diff)
    {
        ...
        if (App::scenes->getEventsEnabled())
        {
            CCbor* ev = App::scenes->createObjectChangedEvent(sim_handle_scene, prop(PropScene::simulationTime).name, true);
            ev->appendKeyDouble(prop(PropScene::simulationTime).name, _simulationTime);
            App::scenes->pushEvent();
        }
    }
...
void CSimulation::_setSpeedModifierRaw(int sm)
{
    ...
    if (diff)
    {
        ...
        if (App::scenes->getEventsEnabled())
        {
            const char* cmd = prop(PropScene::speedModifier).name;
...
{ // Return value is true if the command belonged to hierarchy menu and was executed
    if (commandID == SIMULATION_COMMANDS_TOGGLE_REAL_TIME_SIMULATION_SCCMD)
    {
        ...
        { // we are NOT in the UI thread. We execute the command now:
            ...
            if (App::scene->simulation->isSimulationStopped() && noEditMode)
            {
                App::scene->simulation->setIsRealTimeSimulation(
    ...
    if (commandID == SIMULATION_COMMANDS_START_RESUME_SIMULATION_REQUEST_SCCMD)
    {
        ...
        if (editMode == NO_EDIT_MODE)
        {
            if (!VThread::isUiThread())
            { // we are NOT in the UI thread. We execute the command now:
                App::scenes->simulatorMessageQueue->addCommand(sim_message_simulation_start_resume_request, 0,
                                                                       0, 0, 0, nullptr, 0);
            }
            else
            { // We are in the UI thread. Execute the command via the main thread:
    ...
    if (commandID == SIMULATION_COMMANDS_PAUSE_SIMULATION_REQUEST_SCCMD)
    {
        if (!VThread::isUiThread())
        { // we are NOT in the UI thread. We execute the command now:
            App::scenes->simulatorMessageQueue->addCommand(sim_message_simulation_pause_request, 0, 0, 0, 0,
                                                                   nullptr, 0);
        }
        else
        { // We are in the UI thread. Execute the command via the main thread:
    ...
    if (commandID == SIMULATION_COMMANDS_STOP_SIMULATION_REQUEST_SCCMD)
    {
        if (!VThread::isUiThread())
        { // we are NOT in the UI thread. We execute the command now:
            App::scenes->simulatorMessageQueue->addCommand(sim_message_simulation_stop_request, 0, 0, 0, 0, nullptr, 0);
...
void CSimulation::addMenu(VMenu* menu)
{
    ...
    bool simRunning = App::scene->simulation->isSimulationRunning();
    bool simStopped = App::scene->simulation->isSimulationStopped();
    bool simPaused = App::scene->simulation->isSimulationPaused();
    bool canGoSlower = App::scene->simulation->canGoSlower();
    bool canGoFaster = App::scene->simulation->canGoFaster();
    if (simPaused)
...
#endif
    menu->appendMenuSeparator();
    menu->appendMenuItem(noEditMode && simStopped, App::scene->simulation->getIsRealTimeSimulation(),
...
int CSimulation::setBoolProperty(const char* pName, bool pState)
{
    ...
    if (strcmp(pName, prop(PropScene::removeNewObjectsAtEnd).name) == 0)
    {
        retVal = sim_propertyret_ok;
        setRemoveNewObjectsAtSimulationEnd(pState);
    }
    else if (strcmp(pName, prop(PropScene::realtimeSimulation).name) == 0)
    {
        retVal = sim_propertyret_ok;
        setIsRealTimeSimulation(pState);
    }
    else if (strcmp(pName, prop(PropScene::pauseSimulationAtTime).name) == 0)
    {
        retVal = sim_propertyret_ok;
        setPauseAtSpecificTime(pState);
    }
    else if (strcmp(pName, prop(PropScene::pauseSimulationAtError).name) == 0)
    {
        retVal = sim_propertyret_ok;
...
int CSimulation::setIntProperty(const char* pName, int pState)
{
    ...
    if (strcmp(pName, prop(PropScene::stepsPerRendering).name) == 0)
    {
        retVal = sim_propertyret_ok;
        setPassesPerRendering(pState);
    }
    else if (strcmp(pName, prop(PropScene::speedModifier).name) == 0)
    {
        retVal = sim_propertyret_ok;
...
int CSimulation::getIntProperty(const char* pName, int& pState) const
{
    ...
    if (strcmp(pName, prop(PropScene::stepCount).name) == 0)
    {
        pState = _simulationStepCount;
        retVal = sim_propertyret_ok;
    }
    else if (strcmp(pName, prop(PropScene::stepsPerRendering).name) == 0)
    {
        pState = _simulationPassesPerRendering;
        retVal = sim_propertyret_ok;
    }
    else if (strcmp(pName, prop(PropScene::simulationState).name) == 0)
    {
...
int CSimulation::setFloatProperty(const char* pName, double pState)
{
    ...
    if (strcmp(pName, prop(PropScene::timeStep).name) == 0)
    {
        setTimeStep(pState);
        retVal = sim_propertyret_ok;
    }
    else if (strcmp(pName, prop(PropScene::timeToPause).name) == 0)
    {
        setPauseTime(pState);
        retVal = sim_propertyret_ok;
    }
    else if (strcmp(pName, prop(PropScene::realtimeModifier).name) == 0)
    {
...
int CSimulation::getFloatProperty(const char* pName, double& pState) const
{
    ...
    else if (strcmp(pName, prop(PropScene::simulationTime).name) == 0)
    {
...
int CSimulation::getStringProperty(const char* pName, std::string& pState) const
{
    ...
    if (strcmp(pName, prop(PropScene::simulationState).name) == 0)
    { // Enum
        retVal = sim_propertyret_ok;
