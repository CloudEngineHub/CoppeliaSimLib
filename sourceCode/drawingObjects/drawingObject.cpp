...
CDrawingObject::CDrawingObject(int theObjectType, double size, double duplicateTolerance, int sceneObjId, int maxItemCount, int detachedScriptHandle)
{
    ...
    if (sceneObjId != -1)
    {
        CSceneObject* it = App::scene->sceneObjects->getObjectFromHandle(sceneObjId);
        if (it != nullptr)
        {
...
bool CDrawingObject::addItem(const double* itemData)
{
    ...
    {
        CSceneObject* it = App::scene->sceneObjects->getObjectFromHandle(_sceneObjectId);
        if (it == nullptr)
...
{
    if ((_bufferedEventData.size() > 0) && App::scenes->getEventsEnabled())
    {
...
void CDrawingObject::draw(bool overlay, bool transparentObject, int displayAttrib, const C4X4Matrix& cameraCTM)
{
    ...
    if (_sceneObjectId >= 0)
    {
        ...
        else
        {
            ...
            {
                if (((App::scene->environment->getActiveLayers() & it->getVisibilityLayer()) == 0) &&
